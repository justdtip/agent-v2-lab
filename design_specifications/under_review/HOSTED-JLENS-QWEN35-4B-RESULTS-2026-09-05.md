...
## Addendum, 2026-09-05 23:45: the quantisation gap, measured

Every result above was read on the 4-bit checkpoint. On the Director's approval the bfloat16 checkpoint was pulled and the same probe run on it, paired case by case (`research/records/QUANT-GAP-2026-09-05/`). The workspace readings are the model's: layer similarity identical (CKA within 0.004), corpus statistics within a few percent at every layer with the band coinciding, in-band case readouts correlating 0.79 to 0.98, and, on the statistics EXP-001 rests on, every paired sign test significant on 4-bit significant on bfloat16 in the same direction and the decomposition reproducing exactly. Absolute probabilities differ by up to a factor of two at layers 11, 27 and 28 and the model's own output agrees least (0.66), so results resting on final-layer probabilities carry that caveat; the within-checkpoint decomposition does not. Layer 16 is the exception: its discordant structure changes character between checkpoints and its case ranking is half preserved (Spearman 0.53), so rank-based WP5 and WP7 results at 16, the attention member of the 16/17 pair, carry a checkpoint-dependence caveat stated in their artifacts. The Head's assumption A4 is discharged elsewhere; the band placement needs no caveat.
