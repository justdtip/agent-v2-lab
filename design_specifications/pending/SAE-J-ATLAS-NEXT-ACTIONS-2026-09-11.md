...
2. **The atlas covers 2 of 33 layers.** **Correction, 2026-09-11 09:02Z: the Chief called this "pure extraction, about ten minutes" and it is not.** Each layer entry in the export request requires a prior bridge A2 *result* at that layer with its digest, so an all-layers atlas needs a bridge run per layer first: about four minutes each, roughly two and a half hours of CPU for all thirty-three, and only then the export. Every layer's dictionary is on the card, so nothing must be fetched. The runs are CPU and do not contend with GPU work, and they were started at 09:02Z alongside the call test. The depth curve they produce is the scientific question behind "does it support the full network".
