...
*2026-09-11 UTC, 01:45Z (card clock):* **three narrowings, on the reviewer's re-read of the arrays, applied to the record.** "Any error" reads "the sampled norm- and angle-matched random errors": the comparison is with those distributions, sixteen draws per cell. "The limit is what twenty atoms express, not the encoder" is withdrawn: exact NNLS certifies coefficients on a chosen set, greedy selection does not certify the best set; what stands is that the two procedures tried gain half a point and one point of raw share. The complete-call counts separate malformed outputs from changed valid calls: layer 24's random count is 15 valid changed calls plus one malformed output, the angle-matched 12 plus one; layer 18's nine reconstruction changes are all complete calls. Joined on the 60 unique action positions tested in the model, the lens-score argmax survives in 40 of 60 (layer 18) and 28 of 60 (layer 24) where the model's own survives in 51 and 60 — recorded as the reason lens-score fidelity and behavioural fidelity stay separate outcomes. **For the decision-focused measure's specification (Codex, file-only, specify before implementing):** the declared token set is frozen from the unmodified baseline — the ordered union of the six tool first-tokens and the baseline's top ten — and reused across the reconstruction and every control; the current script measures gaps among the baseline top ten only and does not yet include that union. Outcomes stay separate and are never summed into one number: actual logit-gap changes on the frozen set, next-token switches, complete parsed-call changes, and invalid generations, each with its matched random and angle-matched controls.

*2026-09-11 UTC, 02:00Z (card clock):* **T6 SPEC draft received (`codex/sae-j-behavioural`, `SAE-J-BEHAVIOURAL-2026-09-11/README.md`, uncommitted pending one ruling) — ruled.** Codex found that the reference's same-state *generation* control differs from the baseline generation at row 601 by 9.5 × 10⁻⁶ in the first-token margin with identical text: the generation path (truncated prefix, cache) is not the capture-context forward's kernel path. Ruled: **bitwise equality is a property of the capture-context direct forward and applies there in full** — the unpatched forward reproduces the stored residual bitwise or refuses, and the same-state direct forward leaves every logit unchanged or refuses. **Generation is its own path with its own baseline**: the unmodified generation from `input_ids[:P+1]` is the baseline for the call outcomes; the same-state generation must reproduce its token IDs exactly, and its per-step score differences are measured and reported per cell (max absolute, and on the frozen token set) against a tolerance declared in the config, with no default; the acceptance cell's 9.5 × 10⁻⁶ is then a reported number, not a refusal. The two paths are never compared to each other as if they were one. With that, the SPEC may be committed and implementation begins; report by commit; do not merge.

*2026-09-11 UTC, 03:39Z (card clock):* **the width control closes** (`SAE-J-DEVICE-RUNS-2026-09-10` §5, b1a40c9). On the shares: width at fixed sparsity changes nothing at any layer of either model in or out of domain; density buys a tenth to a fifth of the raw share and drags the lens-score share across the 0.5 line where it was near it; the ratio does not move for any dictionary, as the random-error control predicts. The suite-matched 16k ladder from `resid_post` agrees with the all-layer suite to a few thousandths at matched width and sparsity, so suite is not what the 65k rows measure. On behaviour, at 4B layer 18 on the 32 lowest-margin decisions — the only cells where any reconstruction changed a call — the all-layer 16k `l0_small` changes 9 of 32 calls and the 65k small, 65k big and 262k small change 2, 3 and 2, against 15–20 of 128 matched random draws in every run; the two that do best are at the same sparsity and nearly the same raw share as the one that does worst, and the densest (best reconstruction) is not the best behaviourally. **Ruled for the record:** no ordering by width, sparsity or raw reconstruction error predicts behavioural preservation on this sample, which is the capacity explanation's failure rather than its confirmation; 32 cells at one layer of one model cannot rank dictionaries, and the record says so. The fine-tuning remedy therefore stays unspent: it would optimise a reconstruction objective whose relation to the behavioural result is exactly what these four dictionaries show is not monotone. **What the card's SAE–J work leaves for the next rental:** a larger low-margin sample at both layers and on the 12B, through T6's measure once Codex lands it.
