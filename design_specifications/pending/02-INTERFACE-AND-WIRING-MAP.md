...
- **R40c, standing lift.** See `live/STANDING-LIFT-EXPERIMENTS-2026-09-05.md`: ready-and-validated experiments run without a per-run lift; one model-loading experiment at a time; every result reported to the Director; visualisations are a deliverable. **Status (2026-09-05 late evening): the Proxy relayed the Director's word that a standing lift applies to test runs, with full experiments still under the standing-order lift rules; the Chief's operational definition of test run against full experiment is in the live note's banner, pending the Director's correction.**
