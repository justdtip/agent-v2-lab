...
*2026-09-10 UTC, 12:30Z (card clock):* the v3.4 masking stores now exist — `captures/12b-w3b` (300 rows, 2,373 receipts, 34.0 min) and `captures/4b-w3b-v34` (300 rows, 2,373 receipts, 17.4 min), both admitted in strict mode, the corpus digest read from the capture-time event, the count-matched carrier arm present, every non-control arm reproducing the earlier passes exactly. **Standing request 6's remainder (G6 by an independent oracle, G9 with the real tokenizer, G10's digest branch) is live on these two stores.**
