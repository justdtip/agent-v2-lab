...
How: (1) a process-list check **by mapped library, not by name**: any process the kernel says has `libmlx.dylib` mapped, whatever it is called (corrected 2026-09-05, late: this clause first listed entry-point names, `agent-v2-*`, `run_hosted_lens`, `contrast_reactions`, `mlx_lm`; two of those were artifact-directory scripts that are not in the tree, and a name list only finds the runs that were already careful. The live case, pid 63943 `ctxmax.py`, matched no name and held the model; the library check finds it, issue 83), and (2) a lock file at `outputs/.model-run.lock` carrying the launching session's name, the command and a timestamp, written before the model loads and removed on exit. A launch that finds a live process or a lock younger than the longest expected run refuses and reports, rather than queueing silently. A stale lock (older than the run it names) is reported to the Chief, not deleted by the launcher.
