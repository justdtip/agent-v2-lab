...
- 2026-09-08 (**issue 85 closed on 0b29a85 with five corrections to my own wording; the transcript hole closed, and it was twice the size anyone said**).

  The Chief's three-point amendment plus two further defects found on the page, all verified against `sdpa.json` and `headdim.json` before accepting them, all recorded in a dated append to the record (`a6a63dc`) and fixed on the published page. **None of them moves anything the gate computes.**

  (a) "Fitted where the linear terms have washed out" was exactly backwards. The per-length coefficient falling 155, 127, 113, 106, 103.3 *is* the linear term showing itself; refitting `a·T² + b·T + c` gives `a = 100.5` with `b = 21.3 KiB/token` on all five rows and 100.5 with 22.0 on the two largest. 102.3 carries the remainder and errs high by 1.8 percent, 0.11 GiB at 8,192, which the gate keeps as a bound in the right direction. (b) "Equals the inputs to the byte" at head dimensions 64 and 128 is a cancellation: `inputs_bytes` is exactly 3.000× the bfloat16 inputs at every row of both probes bar the 8,192 one at 2.333×, because the float32 sources of the `mx.random.normal` draws were resident at the baseline read. Withdrawn; the conclusion stands on the Chief's better reason, that an `O(T)` release cannot mask an `O(T²)` allocation. (c) The peak table was raw peaks under a caption saying inputs were subtracted — the fit did use the subtracted values, and the subtracted column's ratios per doubling (3.28, 3.56, 3.75, 3.90) make the point better than the raw column's did. (d) "About 2.6× high" is a GB/GiB slip; 16.02 GiB against 6.453 GiB is **2.48×**. (e) The ceiling is 5,013 as the landed code computes it; `fit.json` said 5,014 and the code is authoritative.

  Ruling carried: the one-block term times the attention-block count with checkpointing off stays, now declared an **upper bound rather than a measurement**, because the backward's transient part does not stack while the forward-retained scores do, so the true no-checkpoint cost is nearer `8 × 1.1 + 2` matrices than `8 × 3.2`.

  **`design_specifications/pending/TRANSCRIPT-GUARDS.patch` (209d6be).** The record-guard rule globbed `*.py` only, so on the day it landed it reported the records tree clean while **nineteen as-run transcripts reached the model and one carried a guard**; sixteen of the nineteen load the checkpoint and twelve are in WP12-BROADCAST-HEADS alone. `python probe.as-run.py.txt` runs a file as happily as a `.py`: the rename closed the import accident and left the launcher open, and nothing said so. `_record_sources` now finds both suffixes with everything downstream unchanged. Eighteen gained the standard refusal. **Two of the eighteen are mine, written into the ATTENTION-TERM record this evening, hours after I landed the rule that exists to stop exactly that.**

  No manifest pinned a transcript's bytes, so the Chief's hash condition had nothing to update in that sense; the stronger claim is the records' own prose calling these files byte-exact, which they no longer are. Each affected record carries a dated declaration with pre- and post-guard digests, TRAIN-COST carries a note that its item (4) holds locally but not tree-wide, and `research/records/GUARD-HASHES-2026-09-08.json` holds all eighteen, verified against disk.

  **A slip disclosed.** Mutation-testing a guard I restored the file with `git checkout`, which reverts to HEAD and therefore to the *unguarded* version rather than to my working state, and then ran it by hand to see the refusal. It loads the checkpoint. It died at line 10 on `sys.argv[1]` before any MLX import, and the process table and lock path were clear immediately after, but that was luck rather than design. The rule runs these files under stub `mlx`, `mlx_lm` and `local_llm_lab` packages precisely so a failed guard cannot reach the model, and running one by hand bypassed the protection I had just built. Redone with a file copy; the digest re-verified against the declaration. General form: to restore a mutated file, restore the working bytes, never `git checkout` — HEAD is a different state and on a guard lane it is the dangerous one.

  Verification: full suite exit 0; `tests/test_repository_rules.py` exit 0 with one new test; the mutation names the file at its line; two guards run by hand exit 1 with the refusal before any import; ruff clean; `git apply --check` 0.

- 2026-09-08 (**the record-guard rule covers the as-run transcripts; eighteen guarded, digests declared**). The Deputy's `TRANSCRIPT-GUARDS.patch` (209d6be) reviewed and landed unamended. The gap was mine to have seen when issue 89's rule landed and the Deputy's to have seen when they wrote two unguarded transcripts into ATTENTION-TERM hours after it: the `.as-run.py.txt` suffix stops a record copy being imported and does nothing about `python` running it, and the rule's discovery was `*.py` only. Checked independently in a worktree: the tree holds twenty transcripts, nineteen reach the model by the rule's own AST test, one (TRAIN-COST's probe) already carried a guard, eighteen gain the standard refusal after their docstring or their `from __future__` line, and the one left unguarded (TRAIN-COST's runner) has no model-reaching site. All eighteen declared digest pairs verified: the "before" against the file at a6a63dc, the "after" against the guarded file. Two guards run by hand refuse with exit 1 before any import. Each affected record carries a dated append with its table of pre- and post-guard digests; the full set is `research/records/GUARD-HASHES-2026-09-08.json`; TRAIN-COST notes that its byte-exactness claim now holds for its own files and not tree-wide. The Deputy's disclosed slip, restoring a mutated transcript with `git checkout` to its unguarded HEAD state and running it by hand, is on record in the patch; it died on a missing argument before any import and the lock and process table were checked clear, and the rule's stub execution exists precisely so that a person never has to be lucky that way.

  Verification: worktree, `tests/test_repository_rules.py` 40 passed, exit 0; full suite 1,708 passed, 14 skipped, exit 0 (the tokenizer-cache warning only); my own mutation, one guard removed from a transcript, named by the rule at its line and restored to its declared digest. Landed in the commit carrying this entry.

- 2026-09-08 (**correction to the entry at `dd095d6`: the DEBT block's fourth site existed, and I asked the wrong question about it**). Appended, never edited, per the 2026-09-07 19:50 ruling.

  That entry says `probes/state_probe.py` "has no manifest write and never had one", cites `git log -S'manifest.json'` as empty, and draws a lesson from it about deleting stale comments. **The premise is wrong.** The block's words were "`probes/state_probe.py:2845` duplicates this helper", and the helper it means is the **atomic writer**, not the manifest write. At `30424d6`, the commit that wrote the comment, line 2845 of that file is `def _atomic_text(path, text)`: a functional twin of `runlog.write_text_atomic` — same-directory temporary with the dotted name and `.tmp` suffix, write, flush, `fsync`, `replace`, `unlink` on failure — sitting at 2940 at HEAD with eight callers and no reference outside the module. The `git log` search really is empty; the question was wrong, not the site.

  **The same error as the two the Chief amended in FORK-SITES this morning: checking a mechanism instead of reading the text.** Three times in one day, and each time the check I ran was well-formed and answered a question nobody had asked. The general form for next time: when a comment or an issue names a thing, quote the sentence and identify its antecedent *before* choosing what to grep for. `duplicates this helper` has an antecedent; `manifest.json` was my substitution for it.

  Delivered as `design_specifications/pending/R21-DEBT-amended.patch` (6e6ebda), superseding the original: `_atomic_text` deleted, its eight callers through `runlog.write_text_atomic`, the two `np.savez_compressed` writers left alone because they are binary and `runlog` has no bytes variant (they duplicate each other within the module, which is worth its own line and not this issue), and the test's fourth-site assertion replaced with an AST check that no atomic *text* writer is defined there — `mode="w"` is what keeps the npz pair out of it — mutation-checked in both directions. Full suite exit 0, ruff 7 before and 7 after, `git apply --check` 0.

  The Chief's checks on the original stand: the interrupt helper fails every write after it is armed, so the config write fails first, the `finally` runs, the health write is attempted through the shim, and only an atomic writer leaves nothing behind. The health test is not vacuous.

- 2026-09-08 (**issue 92 filed and delivered; issue 88 sized and held; two findings about the gate that are nobody's issue yet**).

  **`design_specifications/pending/ATOMIC-GATE-INPUTS.patch` (40483f5), issue 92.** `preflight.write_report` and `provenance.write_provenance` through `runlog.write_text_atomic`. The framing that matters: the preflight artifact is a **gate input, not a sentinel**, and the two fail in opposite directions. A sentinel fails permissively — `guard_dataset_write` reads `manifest.json`'s presence, so a truncated one wrongly allows a later write. This one raises, because JSON that will not parse cannot answer permissively. So a truncated artifact causes no wrong decision; it causes a refusal that looks like a failed preflight, at the start of the run the file exists to permit, with the cause invisible (the file is there and the right size on a listing), and the next move is to re-run the preflight, which loads the model. Its fixture therefore asserts the **previous** artifact survives whole, not merely that the new one is absent. `provenance.json` is the milder one and fails in the way its own contract does not cover: `git_commit`'s "provenance must never kill a run" is about *collecting* provenance and answers "unknown" for each such case, while a failed write leaves a record that parses as nothing. Both fixtures use `test_branch`'s interrupter and both fail against the un-migrated code. Full suite exit 0, ruff 0 before and 0 after, `git apply --check` 0.

  **Issue 88 sized, not started**, per the Chief: it trains, and one model at a time means it is slotted against Codex's fitting rather than started into it. Training 7.2 h across the three depths (1.6 / 2.2 / 3.4), evaluation 5.3 h at 240 tasks each, about **13 hours in six separable blocks**. Projected peaks 9.30, 9.43 and 9.64 GiB, all near 0.53 of the working set, so no R47 window is needed. The full-depth anchor is arm A's own health record — 780 rows in 8,030 s, **10.29 s/row** — rather than a probe figure; top 16's 198 tok/s is interpolated on the model that seconds per token is linear in adapted depth, stated as an input to the sizing and an output of the run.

  Three things the sizing turned up. (1) **Arm A cannot serve as the full-depth arm**: `iters_planned` 1200, `iters_done` 780, status error, verdict incomplete, so all three depths train and the 3.4 h is not a saving that already exists. (2) **`health.json`'s `peak_memory_gb` is gigabytes and the field name says otherwise** — `mlx_lm/tuner/trainer.py:341` is `mx.get_peak_memory() / 1e9` — so arm A's recorded 10.444 is 9.726 GiB, and `runlog.py:714` compares that field directly against `memory_budget_gib`, giving the R26 memory flag a 7 percent margin it did not earn. (3) **The envelope does not model gradient accumulation and this recipe uses 4**: at 2,688 tokens it reads 9.637 GiB where arm A actually peaked at 9.726, under by 0.089 GiB on the one run that matters, and `_calibration_domain_departures` names batch size and checkpointing but not accumulation, so the departure is invisible in the artifact today. Small, and in the wrong direction for a gate. (2) and (3) are findings about the footprint block rather than about 88; neither is filed, and the routing is with the Chief.

- 2026-09-08 (**issues 74 and 92 landed: every JSON sentinel, gate input and provenance record is written whole or not at all**). The Deputy's `R21-DEBT-amended.patch` (6e6ebda) and `ATOMIC-GATE-INPUTS.patch` (40483f5), reviewed together and landed as two commits. The amendment does what the first delivery said could not be done: the debt comment's fourth site, "`probes/state_probe.py:2845` duplicates this helper", was the module's private atomic text writer, a functional twin of `runlog.write_text_atomic` with eight callers; it is deleted, the callers rerouted, the two binary writers left alone as out of scope, and the test now asks by AST that no text twin returns, which a restored twin trips and the binary pair does not. Issue 92 routes the preflight artifact and the provenance record through the same writer. The Deputy's framing holds and is worth keeping: the preflight artifact is a gate input, not a sentinel, and a truncated one fails the safe way, by raising, but the refusal looks like a failed preflight at the start of the run the file exists to permit, and the natural next move re-runs the preflight and loads the model; so the fixture asserts the previous artifact survives whole, which is the live case. Verification in a worktree with both applied: `test_cli`, `test_preflight`, `test_provenance` 195 passed; full suite 1,713 passed, 14 skipped, exit 0.

  Two findings from the Deputy's sizing of issue 88, routed as two issues for them to file and take, in this order. First, the R26 memory flag compares unlike quantities: `health.json`'s `peak_memory_gb` is the trainer's `get_peak_memory() / 1e9`, gigabytes as the name says, and `runlog` compares it directly against `memory_budget_gib`, which is moreover the registry's declared 22 rather than the preflight's resolved 17.76, so the flag cannot fire before the machine swaps and compares in the wrong unit when it does; arm A's recorded 10.444 is 9.726 GiB. The fix records both units in the health summary, compares in one, and takes the resolved budget. Second, the training envelope does not model gradient accumulation and names no departure for it, and arm A, at accumulation 4, peaked 0.089 GiB above the chunkwise envelope at its own row length; the two launch-night diagnostics that would have separated accumulation from a long run's allocator state died at launch with no peak recorded. The issue names accumulation as a departure, adds arm A's measured peak as a calibration point with its provenance so the envelope clears the one run that matters, and settles the cause with two short runs at 2,688 tokens, accumulation 1 and 4, under the lock. Issue 88 stays sized and held behind Codex's box time.

- 2026-09-08 (**issue 82's sizing method and pre-check arithmetic delivered ahead of its hooks**). `design_specifications/pending/WP4-SIZING.patch` (ecc2f89): `probes/read_share.py` with tests, no model, no dependency on 81 landing.

  **(b)'s arithmetic is here; (b)'s data is not.** The share is a pure function of an alpha row and the value vectors it weights, at the interface the ratified WP3 design declares (`y_t = sum_s alpha[t, s] v_s`), so what 81's hooks add is the adapter that yields those arrays rather than the measurement. Checked whether the recorded calibration artifact could stand in: it cannot — `alpha_calibration.json` holds reconstruction errors and per-block and per-position statistics, not alpha rows or value vectors — so the pre-check genuinely waits on 81 and the fixtures are stubs because the artifact is not one.

  Three decisions in the share, stated so they can be disagreed with. **A norm of a sum, not a sum of norms**: summing per-position magnitudes would bound the answer at 1 and answer a different question, and the smallest case that separates them (two opposed sources of equal weight, magnitude reading 0.5, actual read zero) is a test. **A share above 1 is a reading and is not clipped**, because the span's partial sum can point against the remainder and removing it then moves the read further than the read's own size. **A layer reports its maximum head, not its mean**, because the rule asks whether the share is below a threshold at *every* layer and one head carrying the read is the case it exists to catch. The zero rule follows WP3's.

  The sizing applies the plan's rule in the plan's units, returns "not worth running" as an outcome rather than an exception, computes at `alpha / arms` for the four cells read against arm C, and takes a **stable crossing** rather than the first n to touch the target, per WO-STAT-001's correction that discrete tests are not monotone in n. **`matched_share_for` is required with no default**: there is no measured mapping from a norm share to a matched-only share, and a guess inside a sizing function is how an assumption becomes a number nobody can find later; the Head owns it and this records it as an input. Worked with the tests' declared stand-in, a pre-check negligible everywhere sizes against 0.05 → matched 0.70 → **217 points at α/4, power 0.8017**, against this experiment's expected ~42.

  Flagged rather than overridden: issue 82's constraints say Codex implements and the Deputy commits. The work was assigned directly, it is instrument arithmetic rather than the experiment, and it is delivered under `pending` for the Chief's gate like every slice this evening; if it should go back to Codex the design hands over, not the code.

  Verification: full suite exit 0; `tests/test_read_share.py` exit 0 with 11 new tests; ruff 0 on both new files; `git apply --check` 0.

- 2026-09-08 (**box order, and an announced window is a held slot**). Codex's regression resource calibration (minutes, under the primary lock) was refused at launch when a peer's MLX pytest began in the gap between a clear inventory and the lock; no harm, and the third collision of its kind today. From now: a checkpoint launch announced in this log holds the slot from the moment of the announcement until its end line, and no seat starts a pytest run over a file in `_TESTS_THAT_LOAD_MLX` while a window is open, whether or not the lock file exists yet; the closure guard makes the file list mechanical and the launcher's own check remains the last word. Order on the box now, the box being free and Codex having released its scheduling hold: (1) Codex's calibration probe, on retry; (2) the Deputy's two ten-step runs at 2,688 tokens, accumulation 1 and 4, for the envelope issue, after the memory-flag units issue lands so the peaks are read in the right unit; (3) issue 88's first block. Issue 88's "after 87, after 86" was queue order in the Deputy's lane, not a dependency of the ablation on either, and it is lifted: 88 runs in its six separable blocks with Codex's Jacobian timing probe and later fits taking the box between blocks, each announced here. WP4-SIZING (ecc2f89) is under review with an amendment requested; suite on the patched worktree 1,726 passed, 14 skipped, exit 0.

- 2026-09-08 (**74 and 92 closed; the two gate findings filed as 93 and 94; 93 delivered**). 74 closed on `c79b71f`, 92 on `67f52c1`.

  **Issue 93 (`design_specifications/pending/MEMORY-FLAG-UNIT.patch`, 86202b7): the R26 memory flag could not fire before the machine swapped.** Two faults compounded, and the Chief's addition made it worse by one step than I had it. The trainer reports gigabytes (`mlx_lm/tuner/trainer.py:341`, `get_peak_memory() / 1e9`) and `runlog.py` compared that against a gibibyte budget — 7 percent of unearned margin. And that budget was the **registry's declared 22**, not the preflight's resolved `min(22, 17.76)`: `cli.py` handed `HealthThresholds` the spec's `memory_budget_gib` while `preflight.py:954` resolves its own gate against the device. The two gates watched the same run against different numbers by construction. Together the flag fired above 22 GB, **20.49 GiB, 2.7 GiB past a ceiling the hardware cannot exceed without swapping.**

  Fixed with one conversion in one place (`gib_from_gb`); `peak_memory_gib` beside `peak_memory_gb` in the report, the summary and the flag detail, the old field keeping its honest name and every existing `health.json` its reader; the threshold threaded from the preflight artifact rather than re-derived, since a second copy of `min(registry, device)` is how they drift again; and a `--skip-preflight-check` fallback that returns a source string saying it is a fallback rather than passing 22 off as resolved. Fixtures at the real boundary: 17.9 GB is 16.70 GiB and was flagged with a GiB of headroom left, 19.5 GB is 18.16 GiB and was quiet past the ceiling. Both mutations checked; four existing assertions re-pinned deliberately. Arm A's peak re-read as **9.726 GiB** in a dated note on `ARM-A-DIVERGENCE-2026-09-07` (`1ef18b9`), where the next reader will meet it.

  **Issue 94 filed and held for 93:** the envelope does not model gradient accumulation and the recipe uses 4; the envelope reads 9.637 GiB where arm A reached 9.726, under by 0.089 on the one run that matters, and `_calibration_domain_departures` names batch size and checkpointing but not accumulation. Its plan is pre-registered in the issue: name the departure, add arm A's measured point with its provenance, then **attribute** the 0.089 rather than assume it, with two ten-step runs at 2,688 tokens at accumulation 1 and 4 — projected peak 9.637 GiB, 0.54 of the working set, under R47's threshold, R47(b) projection stated before launch, twenty minutes announced first. An open question is recorded there for the Director: these are ten-step diagnostics testing no hypothesis about model behaviour, which reads as a test run under the standing lift, but they go through the training entry point and training lifts keep their own gate. I read them as test runs and will say so at launch; if he reads them as training they wait for his word.

- 2026-09-08 (**issue 93 landed: the R26 memory flag compares gibibytes against the resolved budget**). The Deputy's `MEMORY-FLAG-UNIT.patch` (86202b7), reviewed and landed unamended. The two faults were confirmed on the code before the patch was read: the trainer reports `get_peak_memory() / 1e9`, gigabytes, and `runlog` compared it at a `>` against `memory_budget_gib`; and that budget was the model spec's declared 22, not the preflight's `min(22, 17.76)`, so the flag fired above 20.49 GiB on a machine whose working set is 17.76, unreachable before swapping. Now: one conversion in one place, `peak_memory_gib` beside `peak_memory_gb` in the per-report record, the summary and the flag's detail, the threshold threaded from the preflight artifact's `memory.budget_gib`, and the two boundary fixtures at 17.76 GiB, 17.9 GB quiet and 19.5 GB flagged. Arm A's recorded 10.444 is 9.726 GiB, now in a dated note on its record (1ef18b9). Ruling on the fallback the Deputy raised: with `--skip-preflight-check` there is no artifact, and the patch returns the registry's declaration with a source string that reaches the run log; that stands, because a run that skips the preflight has left the gates by choice and the label makes it visible. The improvement is small and later: expose the preflight's resolution of registry against device as one function and call it from the fallback, so the flag watches the real ceiling even then; filed as a note on 93, not this patch. Ruling on the two accumulation runs of issue 94: ten-step runs through the training entry point at 2,688 tokens that measure a peak and test no hypothesis about model behaviour are test runs under the standing lift as the Director narrowed it on 5 September, the case that lift names; announced here with the projection, they need no further word. Verification in the worktree: test_runlog and test_cli 139 passed; full suite 1717 passed, 14 skipped, exit 0; ruff on the two changed modules 21 findings before and after, none introduced.

- 2026-09-08 (**correction to the entry above, the Chief's own**). The ruff count in the issue-93 landing entry reads "the two changed modules 21 findings before and after". That is the count across all four changed files, taken from the patch header without checking; checked now per file at fb39738^ and fb39738: `runlog.py` 11 and 11, `cli.py` 2 and 2, `test_cli.py` 5 and 5, `test_runlog.py` 3 and 3. Two modules 13, four files 21, none introduced either way. The conclusion stands and the attribution was wrong; recorded because a number written without checking is the error this log has spent the day naming in others.

- 2026-09-08 (**window open: the Deputy's two accumulation arms, issue 94; about twenty minutes**). No Codex window is announced with an open end line and the box is live-free at the moment of writing (no `libmlx.dylib` mapped in any process, no `outputs/.model-run.lock`). This announcement holds the slot until its end line.

  **What runs.** Two arms, identical but for `grad_accumulation_steps`, at 1 and 4. Ten optimiser steps each at 2,688 tokens, chunkwise recurrence at chunk 256, batch 1, gradient checkpointing on, LoRA rank 16 scale 32 on the resolved keys at full depth, AdamW. Arm 1 does ten micro-batches, arm 4 does forty, which is why the second arm is the longer of the two.

  **Invocation arithmetic.** 10 steps × 1 micro-batch × 2,688 tokens = 26,880 tokens for the first arm and 107,520 for the second. At arm A's measured 10.29 s per row that is 1.7 minutes and 6.9 minutes of stepping, plus two model loads at about 20 s each and the allocator warm-up step excluded from the peak; **about twenty minutes for both**, announced first per R47.

  **R47(b) projection.** The first arm is safe by inspection: arm A ran this exact shape and peaked at **9.726 GiB, 0.548** of the 17.76 GiB working set. The second arm is projected from the first before it runs, at 1.25× its measured peak against a 0.60 cap (10.66 GiB); accumulation cannot lower a peak, so the measured arm is the projection's floor and the multiple is its ceiling. If the projection exceeds the cap the second arm does not run and the row records why.

  **The lift, and the reading I am acting on.** These go through the training entry point, and training lifts keep their own gate. I am acting on the standing lift as the Director narrowed it on 2026-09-05: *a performance or hardware benchmark that loads the model but tests no hypothesis about model behaviour is a test run*, and that narrowing exists precisely because an earlier reading held calibrations and instrument pre-checks behind a lift. Ten-step runs that measure a peak and read out no model behaviour are that case. The entry point is the instrument, not the experiment. **Stated here so the Director can overrule the reading rather than discover it.**

  **The reading, pre-registered both ways.** If arm 4's peak exceeds arm 1's by about 0.089 GiB, accumulation carries the gap the envelope is under by and the departure text says by how much. If the two agree, accumulation does **not** carry it, that is the finding, and the allocator hypothesis stays named and unresolved rather than quietly assumed.

- 2026-09-08 (**end line: the Deputy's accumulation arms are finished and the box is free**). Both arms measured, lock released, no process holds MLX. The slot announced above is closed.

  **Result.** Accumulation 1: **9.3822 GiB**, 0.5283 of the working set, median step 30.42 s, 5.1 minutes. Accumulation 4: **9.5050 GiB**, 0.5352, median step 96.78 s, 16.2 minutes. **Difference +0.1228 GiB**, which is 1.02 copies of the LoRA gradient tree (32,464,896 float32 parameters, 0.1209 GiB) — so accumulation holds the accumulator and reuses the incoming gradient's memory, and the derived ceiling of two trees was a ceiling rather than an estimate.

  **The finding is not the one the issue anticipated, and it is sharper.** At the calibration procedure's own condition — one optimiser step per row — the envelope reads 9.6370 GiB at 2,688 tokens and the measurement is 9.3822, so **the envelope clears it by 0.2548**. At accumulation 4 it still clears, by 0.1320. What it does not clear is arm A's 9.7260. So of the 0.3438 GiB between the procedure's condition and arm A, accumulation carries 0.1228 and **0.2210 is unattributed** — the larger part, and it belongs to the long run rather than to accumulation. The allocator hypothesis is not confirmed; it is now the only candidate left standing and it is named as unexplained rather than assumed.

  **Two corrections to the announcement above.** (1) I costed the window from arm A's 10.29 s per row, which is for rows averaging about 1,100 tokens, against 2,688-token rows here; the true cost was 5 and 16 minutes, so about 30 minutes rather than 20. (2) My first R47(b) projection for arm 4 was a flat 1.25× of the measured arm, which refused a run the derived bound shows is safe by a gibibyte; replaced with the bound (measured peak plus two float32 copies of the trainable parameters, 9.624 against a 10.656 cap). A projection loose enough to refuse anything is not a projection.

  **A defect in my own launch check, corrected, and it is not the gap I reported earlier.** I have been writing `lsof -c python | grep -i libmlx` in every launching command this evening. `lsof -c` is case-sensitive and the venv's interpreter is `.../Python.framework/.../MacOS/Python`. Against a live holder of mine, pid 28728: `lsof -c python` found **0**, `lsof -c /python/i` found **1**, and `runlock.running_model_processes()` found 1. So the earlier refusal was not the check-then-lock gap biting me — my check said clear while a holder existed and the **lock** caught it. That explanation is withdrawn. This is R45's own lesson (detect by `libmlx.dylib` mapped, never by process name) violated in the shell while enforced in the code, and the remedy is to call `runlock.running_model_processes()` rather than to write a better grep. **Any seat using the `lsof -c python` idiom has the same blind spot.** The lock refused correctly on both occasions tonight and was the only thing standing between us and a double load.

- 2026-09-08 (**issue 82(c) landed: the read share and the size it licenses, the layer share deciding both branches**). The Deputy's `WP4-SIZING-amended-2.patch` (9c38a38), the third delivery, reviewed and landed. The first went back for four items, one of them a defect: the maximum over heads sized the effect, so a quiet head with a high span share would have licensed a 2 by 2 that can only detect an effect nothing produces. The second went back because my own instruction for that item was wrong: I had asked for negligibility on the maximum and the effect on the layer share, and the two are ordered, the layer share being a magnitude-weighted root-mean-square of the per-head shares and so bounded above by the maximum, which makes negligibility on the maximum strictly stricter and routes the quiet-head case backwards, from a plannable size against 0.05 to "not worth running" against 0.00135. The Deputy derived the bound and checked it on two hundred thousand random layers with zero excess. Landed: the layer share decides both branches, the maximum head share is recorded and never sizes, `single_head_dominates` puts the one-head case in front of the reader, the mapping's source is named on the verdict, an all-undefined pre-check is refused, and `matched_share_for` stays required with no default, the Head's to set. Read against the plan: the share is a norm of a sum, unbounded above and not clipped, with WP3's zero rule; the sizing is WO-STAT-001's stable crossing at the Holm level. Verification in a worktree: 21 module tests, ruff check and format clean, full suite 1738 passed, 14 skipped, exit 0.

  **The Deputy's inventory finding, now R45(b) in the wiring map (5572bf9).** `lsof -c python` is case-sensitive and blind to the venv's framework binary; the Deputy's shell check said "clear" beside a live holder all evening and the lock caught it twice. Reproduced here against a harmless holder of my own that imported MLX and slept: the lower-case spelling found nothing, the case-insensitive one found it, and `running_model_processes()` found it. Every seat's inventory is the lock module's check, in the launching command; the announced-window practice stands beside it for the gap it was written for, which is what refused Codex.

  **Issue 94's two arms, and what arm A's own log adds.** Accumulation 1: 9.382 GiB; accumulation 4: 9.505; the difference 0.123 is one LoRA gradient tree, so the accumulator holds one extra copy and the derived two-tree ceiling was a ceiling. The envelope, 9.637 at 2,688 tokens, clears both; arm A's 9.726 leaves 0.221 to the long run. The trainer's own stdout carries the cumulative peak every ten iterations, and it rose in four discrete steps: 8.163 at 10, 9.328 at 40, 9.527 at 110, 9.605 at 310, 9.727 at 730, with no step at the validation pass at 400, and the last step 0.122, one gradient tree. Whether each step is a new longest row in the data order or growth at a fixed row length is decidable from the row lengths at those four iterations, which the dataset order gives without box time; that is the attribution the 94 patch should carry before it names the allocator.

- 2026-09-08 (**correction to the entry above, the Chief's own, second of the evening**). The reproduction paragraph says `running_model_processes()` found my harmless holder. It did not: the log line reads `runlock: []`. The holder was started from the same shell as the check and so sat in the checker's own process group, which the function excludes by design so that a launcher never refuses itself for its own wrapper; the Deputy's holder, pid 28728, was in another process group and was found. The lsof half of the reproduction stands, zero against one. Two things follow. The inventory belongs in the launching command, not in a shell that has already started something of its own. And I wrote that sentence into the landing before reading the line it describes, the same act I corrected in myself once already tonight and in the Deputy three times; the rule I gave them, read the output before writing the number, applies to the reader of outputs too.

- 2026-09-07T11:11:02.154888+00:00 (**window open: Codex lens replay checks and regression resource calibration retry03; bounded twenty-minute slot**). The Deputy's accumulation end line is present. A fresh primary `runlock.read_lock` returned None and `runlock.running_model_processes()` returned an empty list. This announcement holds the slot until its end line. Source stays in `/Users/daniel.tipton/worktrees/lens-fitting` on `codex/lens-fitting`; this primary-heartbeat append is the operational announcement required by section7, not a source integration.

  First: three tiny installed-model replay integration tests, no checkpoint, over two short split-prefill turns with changed-lens and deliberate hash-failure controls. Second, in a separate process after the tests exit: the previously registered `scripts/lens_regression_preflight.py` retry03, source registration `research/records/LENS-FIT-agentic-regression-2026-09-07/README.md`, prefixes256/512/1024/2044 on the frozen maximum-length fit row with duplicate statistics slots for resource accounting only. Initial inspected bound10.0 GiB, below0.6 of the device's working set or the script refuses; later calls use the registered measured-growth and solve bounds. No fitted artifact, scientific reading, corpus change or full fit. Exact new report `resource-preflight-03.jsonl`; prior refusals remain intact.

  The launch wrapper rechecks the primary lock and mapped-MLX inventory; the normal loader takes the primary model lock. No foreign lock is removed and no foreign process is stopped. R46 pressure supervision applies. The slot ends after this probe or a refusal; issue88 may use the next announced block.

- 2026-09-07T11:18:08.304875+00:00 (**end line: Codex window closed; native replay checks passed, regression probe refused before loading**). The initial tiny-test launch found wt-94 pytest PID49850 and refused before pytest import; the later inventory found wt-82b pytest PID51775. After those holders exited, the guarded native retry ran all three tiny-model replay tests on sourcece219f7:3 passed in1.05s. It exited normally. Exact changed/unchanged-lens controls and abort-error restoration passed; this is not real-checkpoint acceptance.

  The separate regression retry03 then found a new mapped-MLX peer pytest PID55135 in its launching command and refused before invoking the preflight. Only `resource-preflight-03.log` exists; no03 launch metadata, JSONL measurement, checkpoint load or fit. No foreign lock was cleared and no foreign process stopped. This closes the slot; issue88 has no hold from Codex. Calibration's next attempt must use fresh04 paths after an exclusive handoff including MLX-mapping tests. The primary announcement remained present throughout these refusals.

- 2026-09-08 (**issue 94 landed: accumulation named as a departure with its measured cost, arm A's peak in the table, and the envelope clears it**). The Deputy's `ACCUMULATION-ENVELOPE.patch` (228b253), record at eb5b0ee, reviewed and landed unamended. Two ten-step arms at 2,688 tokens under the lock: accumulation 1 peaks at 9.3822 GiB, accumulation 4 at 9.5050, the difference one LoRA gradient tree to two percent, so the accumulator is held and the incoming gradient's memory reused. The envelope clears both; what it did not clear was arm A's long run at 9.7260, and the issue's own premise, that accumulation explained the 0.089, was wrong in its framing: accumulation is a third of a 0.344 gap and the rest belongs to running long. Landed: `grad_accumulation_steps` on the footprint block and named in the departures with the measured number when it is not 1; two measured points at 2,688 with chunk 256, distinguished by their notes, arm A's carrying its provenance and the unit its recorded field does not; the chunkwise offset lifts 0.089 and the ceiling moves 5,013 to 4,984, evaluated from the landed module. Ruling on the caveat: a flat shape's slope is unmeasured at any count, so the caveat is keyed on the shape as well as the count, and adding data no longer silences it. Also on the record: the registry declares accumulation 2 and arm A's config 4, so no arm has ever run the calibrated condition. Outstanding on the record, not the code: arm A's own trainer log shows its cumulative peak rising in four discrete steps at iterations 40, 110, 310 and 730, none at the validation pass, the last one gradient tree; the row lengths at those iterations decide between row length and growth at fixed length without box time, and the Deputy appends that before the 0.221 is called unexplained. Verification in a worktree: `test_preflight` 131 passed; ruff clean; full suite 1,744 passed, 14 skipped, exit 0.

  **The third refusal of the day is mine.** That full suite ran with MLX mapped inside Codex's announced window; I checked the lock and the process table and not the heartbeat, and Codex's calibration retry found my pytest, pid 55135, and refused before loading, after their tiny-test launch had already found the Deputy's suite in the issue-94 worktree. A practice written at 21:00 failed to bind its author by 23:00. That is the case for a mechanism: an issue for the Deputy, the window as a file under `outputs/` that the announcing seat writes and removes, the test collector refusing the MLX-loading files while it exists for anyone but the holder, and the lock refusing a launch over another seat's window. Until it lands, the heartbeat is read before any launch or MLX test run, and Codex's retry has the box before issue 88's first block. This commit also carries Codex's window announcement and end line, which were appended to this file uncommitted; they are Codex's words, preserved here rather than left where the last crash took such lines.

- 2026-09-08 (**issue 94's residual attributed: allocator growth, by measurement against measurement**). The Deputy's `ACCUMULATION-ATTRIBUTION.patch` (bba4c54), comment-only on the arm A calibration point, with the analysis on the record at c82f4cf; landed. The route I proposed, the row lengths at the four step iterations, is closed: the trainer's batch iterator permutes batch order with an unseeded call, so the order is not determined by the run's seed, a reproducibility finding in its own right that the Deputy files separately. The route that works needs no model: the iterator truncates and pads to the cap, so 2,688 tokens is the widest batch the trainer can build, the dataset attains it, the Deputy's ten-step arm measured that worst case at 9.5050 GiB at arm A's own accumulation, and arm A reached 9.7267, an excess of 0.2217 that no row length can supply. Recomputed here from the landed envelope with its quadratic term, anchored on that arm: the five steps imply widths of 2,157, 2,619, 2,697, 2,727 and 2,774 tokens, the first two new longest rows and the last three not buildable, as the Deputy found. Rare discrete steps long after the widest row, on a workload of 77 distinct padded widths, is a pool acquiring block sizes it cannot reuse; the validation pass and the gradient tree were both checked and excluded. The envelope keeps arm A's point as the run the gate exists for. Verification: comment-only, ruff clean, compiles; no MLX test run, the heartbeat read first, no window open, the box free.

- 2026-09-08 (**the fourth refusal is mine too, issue 94 closed, and issue 95 filed for the mechanism**).

  **My suite in `wt-94` refused Codex's tiny-test launch (their pid check found my pytest 49850).** That is the second of tonight's four refusals that I caused, and the failure is precise: I checked the **R45(b) inventory** and not the heartbeat. The inventory answers who is running *now*; a window is a promise about the *next* twenty minutes, and substituting the mechanical check for the protocol one is exactly the mistake available to somebody who has read the practice and means to follow it. The Chief made the same substitution an hour later with the lock and the process table. Codex has now been refused three times in a day and its calibration probe is still unrun.

  **Issue 95 filed on that: the window becomes a file.** `runlock.announce_window` / `end_window` writing `outputs/.box-window.json` beside the lock, with three readers — the test collector skipping the MLX-loading files for a non-holder with the window's text as the reason, the lock refusing a non-holder's launch, and the launching commands owning the file's lifetime. The heartbeat announcement stays as the human record. Stale windows reported and never removed automatically, the lock's own rule. A written practice that failed two careful readers in one evening is the definition of the case for a mechanism.

  **Issue 94 closed on `14f5fca`**, with the attribution at `c82f4cf` and `bba4c54`. The suggested route was closed: `iterate_batches` permutes the batch order with `np.random.permutation` and the trainer's main loop calls it **without a seed** (`trainer.py:273`), so the data order is not deterministic from the config and nothing says which rows sat at iterations 40, 110, 310 and 730. Attributed by size instead, and the decisive half needs no model — the trainer pads to at most 2,688 tokens, the dataset attains that width (14 of 6,648 rows at the cap, 77 distinct widths), the arm measured that worst case at 9.5050 GiB at the same accumulation, and arm A reached 9.7267. Arm A exceeds the widest row the trainer can build by 0.2217 GiB, which no row length can supply. Inverting the individual steps, iterations 10 and 40 imply 2,157 and 2,619 and are new longest rows; 110, 310 and 730 imply 2,697, 2,727 and 2,774 and are not buildable. **The allocator is a finding rather than the last candidate**, and a sharper one: rare discrete steps long after the widest row, on 77 distinct batch widths, is a pool acquiring block sizes it cannot reuse.

  **Sequencing, and a departure from the Chief's order with its reason.** They put issue 95 after 88's first block, on the ground that the block is two hours of waiting. Codex's end line is present and the box is free, but **their probe is still unrun after three refusals**, and starting a 1.6-hour training block now would hold the box against their fourth attempt. So 95 goes first — it needs no box, it is what stops a fifth collision, and it leaves the box free for Codex's retry. 88's first block follows their end line, announced with the projection and the R45(b) inventory in the launching command of the launch itself, not in a shell that has already started something (the Chief's refinement to R45(b): `running_model_processes` excludes the caller's own process group by design, so a seat checking from the terminal it launched a job from will not see that job).

- 2026-09-08 (**Codex's exclusive window: granted, and it excludes MLX test suites**). Codex's remaining lens source is delivered (their branch `codex/lens-fitting` at 452f75c, patch and report under their worktree's `design_specifications/pending/`) and under the Chief's review; their resource calibration is still unrun after three refusals, two by the Deputy's suites and one by the Chief's. The Director relays Codex's request for a handoff that excludes native pytest as well as launches, and it is granted as follows. From the moment Codex's launch wrapper appends its announcement to this log until its end line: no seat runs pytest over any file that reaches `mlx` or `mlx_lm` at any scope, not only the module-scope pinned set; no seat launches anything that loads the checkpoint; the Chief's review of Codex's patch runs its tests under stub `mlx` and `mlx_lm` packages that fail on import, so it maps nothing. Codex runs, serially and each announced with an end line: the resource calibration retry04, then the real-checkpoint Jacobian self-check with its bounds recorded first, the batch-8-first benchmarks, the permitted all-layer fits, the held-out map validation, the hosted-pilot replay identity, the 51 prose continuations under §14, and the profiles; a stop for the Director's ruling if the Jacobian exceeds 25 minutes a layer or the resource limit cannot be met. Issue 95, the window as a file, goes first in the Deputy's lane and needs no box, as they chose; issue 88's first block follows Codex's end line. Until 95 lands, this entry is the mechanism, and every seat reads this log before any launch or test run.

- 2026-09-07T11:35:43.682030+00:00 (**window open: Codex resource calibration retry04; exclusive handoff under5a17860**). Launch wrapper PID76097, source452f75c in `/Users/daniel.tipton/worktrees/lens-fitting`. The launching inventory is clear and the primary lock absent. Until this stage's end line, no other seat runs tests reaching MLX/MLX-LM at any scope or loads the checkpoint, per the Director's grant.

  Registered entry point `scripts/lens_regression_preflight.py`, frozen agentic corpus and pre-existing `LENS-FIT-agentic-regression-2026-09-07/README.md`. Fresh04 log, launch metadata and JSONL. Prefixes256/512/1024/2044, two statistics slots from the same fit row for resource accounting only, followed by one-layer fixed-grid solve timing. Initial inspected bound10.0GiB must clear0.6 of the actual working set; later sizes and solve must pass their registered before-call projections. Approximately minutes; no full fit, lens artifact or scientific reading. Normal primary model lock through load_runtime/load_policy; allocator cache0; kernel-critical pressure on two30-second samples is the intervention signal. No foreign lock or process is touched.

- 2026-09-07T11:40:01.908328+00:00 (**stage end: Codex calibration04 passed; process exited and primary lock released**). Maximum2044-token measured peak 6.8275GiB (0.3844 of working set); all four lengths and one-layer solve below their registered before-call bounds and0.6cap. Agentic regression projects 23.35minutes excluding load/save; no full fit or scientific result. Kernel pressure normal. Evidence `codex/lens-fitting:research/records/LENS-FIT-agentic-regression-2026-09-07/CALIBRATION-04.md`. The granted serial Codex sequence continues with the Jacobian self-check, separately announced after its bounds are frozen; this stage end does not hand the remaining sequence to issue88.

- 2026-09-07T11:47:17.416047+00:00 (**window open: Codex real-checkpoint Jacobian self-check01**). Launch wrapper PID88320, sourceb0470fb, frozen plan9fa35bb395d8c424b982d817f966204dc39bdb8d31f28ed9b9dd716d33dbc558. Primary lock absent and launching mapped-MLX inventory clear. The exclusive grant5a17860 excludes other checkpoint launches and tests reaching MLX/MLX-LM at any scope until this stage's end line.

  Precommitted registration `research/records/LENS-FIT-agentic-jacobian-2026-09-07/README.md`:439-token fit row0, final position438; layers1/16/31,16 unique directions, restore and batch8 broadcast against uncached pre-norm reference. Self atol0.0003/rtol0.003, epsilon stability atol0.003/rtol0.03, frozen before measurement. Initial8GiB inspected envelope against10.656GiB cap, per-workload checks; about a few minutes. Future held-map atol0.01/rtol0.1 is recorded now but not exercised by this stage. No benchmark or fit in this process. Normal primary lock; allocator cache0; R46 pressure supervision. Results or refusal under check-01, no bounds changed after results.

- 2026-09-07T11:54:01.108040+00:00 (**Codex end line: self-check01 inconclusive; exclusive sequence released pending a method ruling**). The checkpoint process exited2, its primary lock is gone and the mapped-MLX inventory is empty. Uncached reference epsilon/half-epsilon stability failed at all three layers under the precommitted0.003+0.03*abs(reference) bound. Maximum absolute response differences: layer1 0.6019568443 at epsilon0.5059779739; layer16 0.1101027727 at epsilon1.8385704041; layer31 0.01689071953 at epsilon8.900575562. Both cached-path outcomes are inconclusive at every layer because their reference is unstable; this does not establish a restore or broadcast fault.

  Original thresholds and all numerical evidence are retained in `research/records/LENS-FIT-agentic-jacobian-2026-09-07/check-01/` on codex/lens-fitting. No benchmark, fit, held-map validation, hosted replay, prose capture or scientific profile followed this failed instrument gate. All observed kernel-pressure samples were normal; no process was killed or foreign lock removed. The exact self-check MLX peak was not exported by this stage, so none is claimed. Regression calibration04 passed earlier and remains valid resource evidence.

  Codex is preparing a diagnostic-only preregistered step-size sweep for the Director's ruling, preserving the original acceptance test and production epsilon rule. **This is the overall Codex end line: issue88 may take the next block.** No model slot is retained while that ruling is pending.

- 2026-09-08 (**issue 95 landed, and Codex's remaining lens source landed on top of it**). Two commits from one combined worktree, suite 1939 passed, 14 skipped, exit 0, native lens tests included, run in the first gap between Codex's windows with the lock absent and nothing mapped, checked in the launching command. The Deputy's `BOX-WINDOW-amended-2.patch` (d204229): the window is a file at one state root shared by every checkout on this machine, read from a linked worktree's `.git` marker, with an operational override; the collector skips every file that can reach MLX at any scope while another seat's window is open, the lock consults the window before the inventory, the holder is a nonce the announcing command exports, overdue windows are reported and never removed. The finding that made the second amendment: as first built, the window hung off the running checkout's root, and every suite that refused a peer on this date ran in a worktree, so the mechanism would have prevented none of the four refusals; verified from inside a worktree before and after. From now the announcement in this log is the human record and `python -m local_llm_lab.runlock announce` / `end` is the mechanism; Codex's wrapper and every launching command call it. Codex's `LENS-FITTING-REMAINING.patch` (their 452f75c): regression and Jacobian fitters, replay with the identity mode, the §14 prose producer, the validation gate and the profiles, reviewed against §3 and §8; the pre-norm tail runs blocks and never the final norm, the reference derivative is the uncached full sequence, the runtime takes the primary lock before the loader import and resolves no-reuse for capture, `J{L-1}` orientation with the unchanged loader's round-trip, §12's penalty, §14's numbers with the ridge overlap disclosed, per-domain base rates, the regression's h=1 rows flagged. Two controls rerun here without a model: the pure set, 229 under stub packages, and the atlas identity control, byte-identical to the committed pilot atlas. One naming defect for Codex's next slice, before any prose profile is read: the atlas labels a prose prompt's tokens "chat", and they are authored text; the label becomes "authored". Their pinned-list hunk was reconciled onto conftest's two lists at landing, declared here: two module-scope files into the load list, the Jacobian tests into the any-scope list, the grep counter-example's held-but-clean list and its counts updated. Codex's calibration passed in their retry04: 6.83 GiB peak at 2,044 tokens, 0.38 of the working set, the regression fit projected at about 23 minutes; their real-checkpoint Jacobian self-check followed under its own window. Three test fixes made at this landing, declared: the Deputy's nested-run test named its probe file `test_preflight.py`, a module name the real file of that name already occupies in a full suite, so pytester's collection hit an import-file mismatch; the probe is `test_window_probe_reaches_mlx.py`. Two of Codex's fixtures, in the profiles and prose tests, asserted at teardown that no `mlx` module existed in the process, which cannot hold in a full suite where other files import MLX at collection and held for Codex only because they ran those files in pure runs; both now assert that the test itself imported none, a before-and-after comparison, with the import guard unchanged. The first full run on the combined tree showed exactly those: one failure and sixty-six errors, none in the code under test. Codex's real-checkpoint Jacobian self-check then failed its own instrument gate at all three layers, ε against ε/2 outside the precommitted bound, and the cause is the inherited step rule taking the norm of the residual over the whole sequence rather than at the perturbed position; the ruling is §15 of the requirements, a per-position step chosen per layer by a preregistered plateau sweep, the regression lens proceeding regardless. This commit carries Codex's window and stage-end lines, appended uncommitted.

- 2026-09-08 (**issue 88's recipe knob landed: `train.lora_layers`, and the band coverage recorded at every depth**). The Deputy's `ADAPTER-DEPTH.patch` (014710f), reviewed and landed unamended. Until now `cli.py` set the LoRA config's layer count to the model's unconditionally, so every adapter this repository has trained covered all 32 layers and the ablation had nothing to vary; the knob is the number of top layers adapted, absent meaning all, validated rather than clamped so a typo refuses before the weights load. With it, `_band_coverage`: a split pair is not a covered pair under R41d, and on the 4B's five pairs the top 8 cover one whole (27/28), the top 16 cover three whole and split 16/17 with 17 adapted and 16 not, recorded in the run log and the provenance at every depth including full so nobody infers coverage from the layer count. The pairs come from the registry through a new declaration-only field on the spec, the validating reader unchanged. Verification in a worktree: test_cli and test_probes 200 passed; full suite 1942 passed, 14 skipped, exit 0; ruff on the four changed files no new findings. The Deputy launches 88's first block on this landing, announced through the window mechanism.

- 2026-09-08 (**window open: issue 88 arm 1, the top-8 depth block; about 100 minutes**). Announced through the file as well as here, `runlock announce --seat deputy --purpose '88 block 1' --minutes 100`, so the collector and the lock both hold the slot rather than only the reader of this log.

  **What runs.** `configs/agent_v2e_qwen35_4b_top8.yaml` (35aa323), which differs from arm A's recipe by exactly two lines — the output directory and `train.lora_layers: 8` — verified by loading both and diffing. 1,200 rows at batch 1, accumulation 4, cap 2,688, chunkwise at chunk 256, gradient checkpointing on, LoRA rank 16 scale 32, 400 steps of accumulation. Adapters on the top 8 layers only, which is **1 of 5 declared band pairs (27/28) whole and 4 outside**, recorded by the run itself in the log and the provenance.

  **Invocation arithmetic, both ways, and I use the larger.** From arm A's own measured 10.29 s per row at full depth, scaled by this issue's measured throughput ratio 276/126: 4.70 s per row × 1,200 = **1.57 hours**. From tokens, 1,200 rows × ~1,100 tokens ≈ 1.32 M tokens at 276 tokens/s = **1.33 hours**. The announcement is 100 minutes, and the two estimates differ because the ratio was measured on 2,688-token rows while this arm's rows average about 1,100.

  **R47(b) projection.** The landed envelope reads **9.726 GiB** at 2,688 tokens for chunkwise, which now includes arm A's own point. This arm's backward is lighter by the depth term issue 88 measured — 1.04 against 0.91 MiB per token at 32 against 8 adapted layers — so 0.13 × 2,688 / 1,024 = 0.34 GiB less: **about 9.39 GiB, 0.53 of the 17.76 GiB working set**, under R47's 0.60 threshold. Training runs are exempt from the window rule in any case and are announced with their duration first, which is what this is.
...
- 2026-09-08 07:58 local (**the gap arrived at 23:57 and the box sat idle through the night: the window outlived its holder**). Issue 88 arm 1 finished at 23:57 local (13:57Z): 1,200 of 1,200 iterations in 100.7 minutes, status ok, verdict warnings with 29 loss-spike flags from iteration 310 on, best val 0.01113 at iteration 799 and 0.1395 at 1,199, peak 8.950 GB = 8.335 GiB (0.469 of the working set), chunkwise throughout, adapters at 400, 800 and 1,200. The lock released with the process. The window file did not: it still names pid 20550, which is not running, against an expected end of 13:57Z. `status` reports it overdue; by the rule it is reported and never removed, and `end` is nonce-guarded to the seat that opened it. So the Chief's watch, which waits for the window file and the lock to both be gone, has been waiting eight hours on a window nobody holds, and the Deputy's `runlock end` at the end line did not happen because the Deputy's session went idle before the end line arrived. Nothing landed, and the box has been empty since 23:57. Routed now: the Deputy ends the window and posts the end line; the watch then runs the suite and lands 96-amended and WINDOW-EXTEND; arm 2 follows, Codex having announced nothing (their branch moved to 5f7a202 overnight, diagnostic work, no window). Mechanism issue to the Deputy: a window opened around a run should be closed by the wrapper that ran it, on exit, not by a seat that has to be awake afterwards; and `status` should say when the holder pid is dead, since overdue by an hour and orphaned are different facts. Eight idle hours is the cost of the first; the second is what would have let anyone see it.
...
- 2026-09-08 (**a teardown instruction that was correct when written and wrong when it arrived; nothing was torn down**). Recorded before the arm-1 report because it is the more important of the two.

  I was asked, in good faith and on a correct reading of an earlier state, to end my orphaned window with a supplied nonce. **I read the window file from disk first and it is not mine.** It names `Codex GPT-6 Astra`, purpose "Approved original-scale epsilon diagnostic01", opened 2026-09-07T21:59:22Z, pid 29649 **alive**; `outputs/.model-run.lock` is held by pid 29675 running `lens_epsilon_diagnostic.py`, and the inventory reports one holder. Codex has adopted the mechanism and is running under it now. My orphaned window was cleared by somebody between that message and this reading; I did not clear it.

  **Nothing was ended.** The supplied nonce is my old window's and does not match the file's, so `end_window` would have refused on the guard. The dangerous path was not the command I was given but the obvious alternative — deleting the file by hand — which would have released a watch that fires a full suite the moment the file disappears, straight into Codex's live diagnostic.

  **The rule that follows, proposed for the wiring map: a launch or a teardown reads the box state from disk, never from a message, however recent.** A message is a claim about the past. The file is the present. This is the same shape as R45(b) — the inventory is `running_model_processes()`, not a name-based grep — one level up: the *authority* for box state is the state, not a report of it.

  **And the failure that put me here is mine.** Arm 1's block finished at 13:57Z and my `runlock end` never ran, because I tied a window's lifetime to my seat being awake. The window outlived its holder by eight hours and the box sat idle through the night. The mechanism I built has no way to notice a dead holder: overdue and orphaned are different facts and it only knew the first. The `runlock run` verb — announce, run, end on exit whatever the status — plus a `status` that reports a dead holder pid, is the fix, and it is mine to write before arm 2 is announced.
...
- 2026-09-08 08:10 local (**correction to this morning's entry: the pid in a window opened from the shell is the announcing command's, and it is always dead a second later**). My entry at fa271a4 and issue 97 both treat "pid 20550 is not running" as the evidence that the Deputy's window was orphaned. The pid `announce_window` records is `os.getpid()` of the process that called it; from `python -m local_llm_lab.runlock announce` that is a python process that prints the export line and exits, so every window opened from the shell shows a dead pid from its first second. My own suite window this morning read "pid 35063, not running" while the suite it covered was running. Codex's window showed a live pid only because their wrapper calls the function in-process and stays up. The Deputy's window was orphaned all the same: its block's lock released at 13:57Z, its expected end passed by eight hours, and nothing of that seat was running; but the pid was not the witness. Issue 97's second item is amended on the issue: the CLI records the parent pid, the shell that evaluated the export and holds the token; `run` records its child's; `announce_window` takes the pid as an argument; and only then can `status` say "holder not running" and mean it. Two more facts for the record of the night. `announce_window` is an exclusive create and refuses when a file exists, so Codex's window at 21:59:22Z could not have replaced the Deputy's; the Deputy's file was removed by hand between my reading at 21:54Z and that announce, not by the Deputy and not by any seat's `end`, whose guard would have refused. The Director's word at the time was "ya snooze ya lose", which I read as the ruling that a window whose seat has gone idle past its end lapses; if that is the rule, it is his to state in the wiring map and it answers the question issue 97 left for him. This entry carries, uncommitted before it, the Deputy's entry on the teardown they rightly refused and Codex's end line for diagnostic01.

- 2026-09-08 08:15 local (**the landing suite found the tree red: the top-8 config landed without the config pin, and the pin now has it**). The first full suite of the morning, run as holder of my own eight-minute window (22:04Z, closed by the shell on exit): 3 failed, 1,946 passed, 14 skipped. All three failures are one cause: `configs/agent_v2e_qwen35_4b_top8.yaml` landed at 35aa323 at 22:16 local last night, four minutes after the last full suite on the tree, and the two tests that pin the list of configs with split tables, so that no run config joins the repository unswept by the disjointness check, were not told; no suite ran on the tree between that commit and mine. Ruff reads 85 on the tree and 85 in the worktree, the same findings, none from the patches. The pin gains the config in both files, in this commit ahead of the landings; the disjointness check then swept the top-8 config and passed, as it should for a config differing from arm A's by two lines. Rerun under a second window (22:09Z): 1,949 passed, 14 skipped, exit 0. The rule this restates: a config is code for the suite's purposes, and landing one is a landing.

- 2026-09-08 (**issue 96 landed: the trainer's row order is a function of the config**). The Deputy's `BATCH-ORDER-SEED-amended.patch` (0243c5c), superseding b9529b2, reviewed and landed unamended. The library's batch iterator permutes batch order from numpy's global state and the training loop passes it no seed, so until now two runs of one config could train on different orders and issue 94 could not say which rows sat at the iterations where arm A's peak stepped up. The stage now seeds numpy from the run's declared seed immediately before the trainer is entered; the provenance carries a `batch_order` block that names the seed, states that records without it cannot be replayed, and says that a resumed run re-seeds from the permutation's start and so is replayable only from its own start, with the resume path recorded. A test pinned against the library's own source fails the day a release seeds its own order. The first delivery's pin parsed the call up to the first closing parenthesis and would have passed silently against a source that wrapped an argument in a call before the seed; the Deputy demonstrated that silent pass against such a source before fixing it, and the pin now finds every call by AST and reads its keywords as keywords, with a second test that exercises the pin's own failure mode. A pin's one forbidden failure is the silent pass. Verification in a worktree, in the gap after 88's first block: 1,949 passed, 14 skipped, exit 0.

- 2026-09-08 (**the window gains extend, and a skip under someone's window now says so on the terminal**). The Deputy's `WINDOW-EXTEND.patch` (0d9fc2f), reviewed and landed unamended on top of 96. `runlock extend --minutes N`, holder-only and nonce-guarded, rewrites the expected end in place and records each extension, so a mis-costed block is corrected without the file ever leaving and the slot never opening; the Deputy's 131-minute block was left as announced on that reasoning. The terminal summary now writes a "box window" section when a run had files skipped under another seat's window, naming the holder and the count and saying the run did not exercise the files that can reach MLX; silent when no window is open or nothing covered was collected. The reason is the Chief's slip of this evening, an exit code read as green from a run whose whole CLI file the collector had skipped. Seen firing here against the Deputy's live window from a non-holder's run: 73 skipped, the section printed. The suite also gained protection from the machine's own window, redirected to a temporary path for the whole run as the lock already is, after three lock tests failed the first time a real window existed. Verification in the same gap and worktree as 96's: the same suite line.

- 2026-09-08 08:35 local (**arm 1's end line reviewed: the memory finding stands, the speed ratio is 2.04× like for like, and the quality question goes to the paired eval**). The Deputy's report on issue 88, recorded at 789d796, checked against the run's own logs. Throughput: the report's 2.13× divides arm 1's median over 120 reports (0.2070; the train log gives 0.2065) by arm A's wall-clock average (780 rows in 8,030 s, 0.0971), which are different estimators. Like for like it is 2.04× either way: medians 0.2065 against 0.101, or wall-clock 1,200 in 6,044 s (0.1985) against 0.0971. Still close to the probe's 2.19×, so "the ratio holds" stands with the smaller number. Memory: arm A 9.726 GiB against arm 1 8.335, allocator growth removed (0.221 against 0.046), leaves 1.216 GiB over 24 removed layers at 2,688 tokens, 0.0193 MiB per token per layer against the 0.0054 the probe fitted, 3.6×; the arithmetic checks. The issue's sentence that depth cannot buy meaningful memory is wrong by that factor; depth buys a seventh of the peak at this recipe. Ruling: the report carries the hypothesis for the factor, that with no trainable parameter below the lowest adapted layer the backward stops there and the frozen layers' activations need not be retained, so 0.0193 is a per-retained-layer cost, and it says under what conditions the probe's 0.0054 was measured. Arm 2 tests it with a projection stated in its announcement, per R47(b): top 16 retains 8 more layers, 8.335 + 8 × 0.0193 × 2,688 / 1,024 = 8.74 GiB before allocator growth. Near 8.74 says the term is per retained layer; near 8.34 says the saving is something else. Val: 0.721, 0.194, 0.011, 0.140 on 48 rows with 29 loss-spike flags; the Deputy's reading, ordinary overfitting with the best point as suspect as the last, is accepted, and selection at the 800 checkpoint is the ablation's rule for its own comparison. It does not answer 88's quality question, which the paired-task eval answers as it did for arm A (base 15, adapter 5, of 19). Routed: arm 1's adapters at 800 and 1,200 through the same paired eval, as the first 88 box job when the Director releases the box from Codex's lens work, ahead of arm 2, because a top-8 adapter that is also worse than base changes what arm 2 is for. Arm A's record confirms it cannot be the full-depth arm (validation at 1 and 400 only; stopped at 780), so all three arms train. The Deputy owns the red tree at 35aa323 and has said they will not commit to the tree again without the suite that covers it, which is R49 as written.

- 2026-09-08 (**arm 1's evaluation prepared and NOT launched; a distinction my own R45(c) needs**).

  **The disk says the box is free** — no window, no lock, no MLX holder, read just now. **I am not launching**, because the Chief's word is that the box is held for Codex by the Director and released by the Chief's message, not by the file.

  That is a limit on R45(c) worth stating, because I wrote the rule and it does not say this. **The disk is the authority on the box's *state*. It is not the authority on *permission*.** R45(c) stops a seat acting on a stale claim that the box is busy or free; it does not turn an empty file into a licence the Director has withheld. A free disk and a withheld release means wait, and reading the disk is what tells me the wait is not the file's doing.

  **The command, prepared and held.** Arm 1's adapters at 800 and 1,200 through the same paired evaluation arm A took, so the quality question this issue asks is answered on the same instrument as the divergence report:

  ```
  eval "$(python -m local_llm_lab.runlock announce --seat deputy \
      --purpose '88 arm 1 quality: top-8 adapters at 800 and 1,200' --minutes 240)"
  trap 'python -m local_llm_lab.runlock end' EXIT
  agent-pipeline --config configs/agent_v2e_qwen35_4b_top8.yaml eval \
      --adapter outputs/agent-v2e-qwen35-4b-top8/adapters/0000800_adapters.safetensors \
      --split test --limit 180
  agent-pipeline --config configs/agent_v2e_qwen35_4b_top8.yaml eval \
      --adapter outputs/agent-v2e-qwen35-4b-top8/adapters/0001200_adapters.safetensors \
      --split test --limit 180
  ```

  `trap ... EXIT` is the stopgap until issue 97's `run` verb lands, and it is the shape 97 makes into a verb: the window's lifetime tied to the process rather than to my seat being awake, which is the failure that cost the group last night.

  **Duration and R47(b).** The only 180-task evaluation on this model took 4,760 s, so two are about **2.6 hours** and the window is announced at 240 minutes. Evaluation is inference, not training: the peak is a load plus a cache, far below the 0.60 threshold, and it takes no gradient. The 800 checkpoint goes first, because it is the one the ablation's own rule selects and the one whose result changes what arm 2 is for.

  **Why before arm 2**, per the Chief's ruling: a top-8 adapter that is also worse than base changes what arm 2 is measuring. Arm A's adapter was worse than base at 19 tasks (base 15, adapter 5), and that divergence is unexplained; if arm 1 shows it too, depth is not the variable to be sweeping.

- 2026-09-08 09:20 local (**box released to the Deputy for arm 1's paired evaluation, on the Director's word; the question is which checkpoint is best**). The Director wants the answer before deciding a change of direction. Order given: checkpoints 800, then 1,200, then 400, each through the paired-task evaluation of the divergence record on its 19 tasks first, the base's greedy results reused from that record and said so, the best checkpoint extended further into the test split only after all three have a tally. One window for the whole job, announced through the file with `end` trapped on exit; the Deputy's estimate was 2.6 hours for two checkpoints, so about four for three. Arm 2 stays held; Codex's regression fit waits on the Director's word. The report the Director asked for is one line, which checkpoint and by how much, with the per-checkpoint tallies and defect classes behind it.

- 2026-09-08 09:40 local (**WINDOW-RUN (issue 97) reviewed and returned: a vacuous test, proven; SIGTERM leaves the window; the separator stripped too widely**). The Deputy's `WINDOW-RUN.patch` (80c0711), applied in `~/worktrees/window-run` at HEAD, 115 pure tests passed under stubs against a temporary state root, ruff check clean; the format diff is in pre-existing code. Three amendments required. First, the test that `run` ends its window cannot fail: the suite's autouse fixture redirects `default_window_path` to a temporary directory, so the env override the test sets is never consulted and the asserted path never holds a file. Proven with a child that looked for the file while `run` held it: absent at the asserted path, present at the redirected one. The fix is to assert on the path the code uses and have the child prove the file existed while it ran. Second, `finally` does not run on SIGTERM or SIGHUP, so a killed wrapper leaves its window as `announce` did last night; the lock's `_install_signal_release` is the pattern, with the signal forwarded to the child, and the test goes through the CLI as a subprocess. Third, every `--` is stripped from the command rather than the leading one. Smaller: a refused announce is a traceback at the CLI; a signalled child's negative code reaches `sys.exit`; pid reuse belongs in `holder_state`'s docstring. The rest reads right: three holder states, orphaned and overdue as separate facts, the parent pid from `announce`, the child inheriting the token. Landing waits for the gap after the Deputy's evaluation window. Off-box, ahead of issue 80.

- 2026-09-08 09:35 local (**arm 1's three checkpoints on the 19 divergence tasks: 13, 18, 18 against the base's 15 and arm A's 5; the depth arm is the first adapter to beat its base**). The Deputy's evaluation window (23:10Z to 23:28Z, three runs of about five minutes each, not the hour estimated) with the base's greedy results reused from the divergence record. Checkpoint 400 passes 13 of 19, below the base, with 28 integrity violations across 5 trajectories and its failures in the calculate-bearing families. Checkpoint 800 passes 18, no loops, 9 violations across 3 trajectories, its one failure the aggregate report with no finish call. Checkpoint 1,200 passes 18, its one failure update-0004 by a verbatim-copy loop to exhaustion, which is arm A's failure mode appearing in this adapter for the first time at the later checkpoint, with 21 violations in that one trajectory. Both 800 and 1,200 pass all four tasks the base fails. Read from the trajectories, not the Deputy's report, which is still to come. The 19 tasks cannot separate 800 from 1,200; ordered: both on the full test split, 800 first, one window, then the report on issue 88 with the one line the Director asked for. What this changes: issue 88 was framed as a speed lever with quality the open risk, and instead restricting the adapter to the top 8 layers is the first change in the programme to produce an adapter better than its base on these tasks, where full depth at the same recipe produced one worse than it. The Wilson interval on 18 of 19 runs 0.75 to 0.99, so the margin over the base is plausible rather than certain until the full split is in; the margin over arm A is not in doubt.

- 2026-09-08 09:45 local (**arm 1's paired evaluation, reported: checkpoint 800, 18 of 19 against the base's 15 and arm A's 5; the tie with 1,200 broken on the kind of the failure, not the count**). Three evaluations in one window, 23:10Z to 23:28Z, greedy, `--limit 19` on the divergence record's own tasks — the first 19 of the test split, so the set is reproduced rather than approximated — in the order set: 800, 1,200, 400. **The base was not rerun**; its greedy rows are the first 19 of `outputs/agent-v2/evals/base-test.json`, the rows the record itself used, and read back they give the record's 15. Five minutes a checkpoint, not the 2.6 hours I estimated, because arm A's adapter spent nearly every task looping to the 24-step ceiling and arm 1's does not — an estimate off by a factor of thirty because it was taken from a run whose slowness was the defect under study.

  400 passes 13, 800 passes 18, 1,200 passes 18. Both 800 and 1,200 pass all four tasks the base fails and lose one it passes. 800's single failure is the aggregate report: it reads six metrics, computes 276, writes `grand_total=276` into the file, then emits prose with no tool call where `finish` belongs — no repeat, no loop, the answer correct and never submitted. 1,200's is `test-update-0004-clean`, a verbatim re-issue of a call that had just succeeded, run to the ceiling, carrying all 21 of that checkpoint's integrity violations. **That is arm A's dominant defect appearing in this adapter for the first time**, so the extra 400 rows buy no pass and reintroduce what the depth restriction was suppressing. Defect classes — 400: required tool unused ×3, truncated tool call ×2, malformed argument ×1; 800: truncated tool call ×1; 1,200: re-issued call after success ×1. Two are new to arm 1 and are named rather than folded into arm A's six. **No checkpoint abbreviates a path**, which was 3 of arm A's 14 failures.

  **The reader was checked before it was used.** `paired_tally.py` reduces arm A's markdown transcripts and arm 1's eval JSON to one shape so a single rule runs over both; on arm A it returns 5 passes, 11 base-pass/adapter-fail pairs and 12 of 14 lock-ins, which are the divergence record's three headline figures, and it agrees with 13 of its 14 hand labels. The one miss is the class that needs the task's expected files, which no trajectory carries. A tally computed by a new rule and a tally computed by a hand reading are not the same evidence until the rule has reproduced the hand reading once.

  **What the numbers do not support.** Nineteen tasks: Wilson on 18 of 19 is 0.754 to 0.991, on the base's 15 of 19 is 0.567 to 0.915; five discordant tasks, four to the adapter, exact paired p = 0.375. The margin over the base is plausible, not established. The margin over arm A is not in doubt.

  **A launch fault worth the next seat's time.** `mlx_lm.load_adapters` takes a **directory** holding `adapter_config.json` and `adapters.safetensors`, never a bare checkpoint file; my first launch passed `0000800_adapters.safetensors` and died three times in four seconds on `NotADirectoryError` for `<file>/adapter_config.json`. The window opened and closed cleanly by the trap and no model loaded, so the cost was four minutes. Each checkpoint is now a directory under `outputs/agent-v2e-qwen35-4b-top8/checkpoints/`, config copied and weights hard-linked, which also gives `stage_eval` distinct stems so three runs do not overwrite one file. Record and trajectories at `research/records/ADAPTER-DEPTH-QUALITY-2026-09-08/` (7188603, f898f47); the answer is on issue 88.

  **Running**: checkpoint 800 across the full 180-task split, one window at 90 minutes with `end` trapped, from 09:31 local, about an hour at the observed rate. 1,200 follows in a second window. Arm 2 held. Next off-box: issue 80.

- 2026-09-08 09:50 local (**the Deputy's arm 1 quality record accepted: checkpoint 800, on the kind of its failure**). `research/records/ADAPTER-DEPTH-QUALITY-2026-09-08/` (7188603, f898f47) and the report on issue 88 agree with the Chief's reading of the trajectories figure for figure. The reader, `paired_tally.py`, was run on arm A's record before arm 1 and returned its three headline figures (5 passes, 11 pairs, 12 of 14 lock-ins) and 13 of 14 hand labels, the miss needing a file list the trajectory does not carry; that is the R52 standard and the reason the new tallies are taken. Exact paired p against the base 0.375 on five discordant tasks, four to the adapter: plausible, not established, and the full split now running is what establishes it. Two defect classes new to the taxonomy are named rather than folded in, truncated tool call and required tool unused; no checkpoint abbreviates a path, which was three of arm A's fourteen. A checkpoint is evaluated as a directory with its config, not as a weights file; the wiring map takes that when 88 closes. Order: full split for 800, then 1,200, then the WINDOW-RUN amendment, then issue 80; arm 2 held.

- 2026-09-07T23:43:47.018585+00:00 (**Director-authorized concurrent diagnostic: Codex position-step01 begins**). The Director explicitly overrides single-load exclusivity for this diagnosis and conditional retry. Existing Deputy evaluation, primary model lock and window stay intact. Only the Codex child uses a private record-directory lock; 6 GiB allocation limit, cache0, 20-minute diagnostic deadline, pressure supervised, no foreign signals. Frozen §15 grid, original bounds, source and registration in `codex/lens-fitting:research/records/LENS-FIT-agentic-jacobian-2026-09-07/`. This is a concurrent exception, not an exclusive window or general release.

- 2026-09-07T23:55:26.323944+00:00 (**Codex concurrent diagnostic position-step01 ends**). {"ended_utc": "2026-09-07T23:55:26.323531+00:00", "exit_code": 0, "reason": null, "elapsed_s": 699.3129321250017, "owned_child_exited": true, "private_lock_remaining": false} Primary lock/window and foreign processes were not modified; this end line does not release their ownership.

- 2026-09-08 10:00 local (**WINDOW-RUN-amended (issue 97) reviewed and approved for landing in the next gap**). The Deputy's `WINDOW-RUN-amended.patch` (41fc725) against 3e894a8, applied in `~/worktrees/window-run`; 119 pure tests under stubs against a temporary state root, ruff check clean, the format diff untouched and pre-existing. All three amendments are in and each was proven to fail without its fix rather than asserted. The `run` test takes the unredirected window path, asserts that `default_window_path()` is the path it goes on to check so the two cannot drift apart silently again, and runs `/bin/test -e` on the window so the child's exit status is the evidence it existed while the command ran; the Deputy ran the old body against a build with the `end` removed and it passed, the new body fails on the same build. SIGTERM and SIGHUP take the lock's `_install_signal_release` shape with the child relayed the signal and given five seconds, tested through the CLI as a subprocess: the wrapper is killed, the window and the child are gone, and with the handler disabled the window survives, which is last night in one assertion. Only the leading separator is stripped. The smaller three are in with tests: a refused announce or run is a report on stderr with the other seat's window untouched, a child killed by signal N exits 128 + N, and `holder_state`'s docstring says a live pid is evidence of a process, not proof of the holder. One note for the record rather than a change: after the five-second grace a child that ignores the signal is left running with the window closed; the box stays protected because that child holds the lock itself, and the docstring could say so. Landing needs the full suite, which reaches MLX, so it waits for the gap after the Deputy's full-split evaluations; arm 2 will not run, but every seat's next window should open under `run`. This entry carries Codex's uncommitted end line for position-step01, before it.

- 2026-09-08T00:04:19.654553+00:00 (**Director-authorized concurrent fit attempt: Codex fit-retry01 begins**). The Director explicitly overrides single-load exclusivity for this diagnosis and conditional retry. Existing Deputy evaluation, primary model lock and window stay intact. Only the Codex child uses a private record-directory lock; 6 GiB allocation limit, cache0, unchanged 25-minute-per-layer gate and 14-hour outer deadline, pressure supervised, no foreign signals. Frozen corrected rule c=0.03; fresh original self-check, batch8-first benchmark, full fit only if both gates pass; source and registration in `codex/lens-fitting:research/records/LENS-FIT-agentic-jacobian-2026-09-07/`. This is a concurrent exception, not an exclusive window or general release.

- 2026-09-08T00:08:32.616891+00:00 (**Codex concurrent fit attempt fit-retry01 ends**). {"ended_utc": "2026-09-08T00:08:32.616190+00:00", "exit_code": -15, "reason": null, "elapsed_s": 252.97233616599988, "owned_child_exited": true, "private_lock_remaining": false} Primary lock/window and foreign processes were not modified; this end line does not release their ownership.

- 2026-09-08 10:25 local (**BAND-PAIRS-FROM-THE-FUNCTION (issue 86) reviewed and approved, the registry entry awaiting the Director; WINDOW-RUN-amended's docstring change confirmed docstring-only**). The Deputy's patch (fc0e300) in `~/worktrees/band-pairs` at HEAD: `kind_matched_layer_family` pairs each in-band layer with the nearest layer of the opposite kind in either direction, as EXP-003 generalised §3.5 on 2026-09-05 and the function never followed; an equal-distance tie is answered from `probes.partner_tie_breaks`, a recorded ruling, and a tie with no ruling is broken to the lower index and reported in `unrecorded_ties` and in the reason string R34 quotes into every artifact. The check script beside the patch passes with `mlx` absent: the band's five recurrent members 13, 17, 19, 23 and 27 each have a unique nearest attention layer, so the band's five pairs come out of the function with no tie anywhere, identical to `probes.live_lens_pairs`; the one recorded ruling is EXP-003's 17 over 15 for layer 16, and without it the function returns the old wrong 15 and says so. The Deputy's first draft wrote four tie-breaks into the registry and withdrew three as guesses, which is the failure the field exists to prevent, caught by its author. C4's pinned family becomes ten layers, 17 joining, with the derivation test and the literal kept as the pair issue 62 wanted. 40 pure tests pass; `test_jlens.py` imports `mlx.core` at module scope and runs with `test_jspace_sweep.py` in the gap, in the same window as WINDOW-RUN's landing suite. The registry hunk, `partner_tie_breaks: {16: 17}` in `configs/models/qwen35-4b.yaml`, is the Director's to approve; the patch does not land before his word. Rulings that follow: issue 80 becomes a check of `live_lens_pairs` against this function rather than a moved or duplicated literal, since the band is now derivable; R40b's set 13, 16, 20, 24, 28 does not reproduce the band through the function because the in-band ceiling is round(32 × 5/6) = 27, which is named and left, nothing depending on it. WINDOW-RUN-amended's change at fe4651c is one docstring hunk, confirmed by diff against the reviewed copy, re-applied, 57 tests pass; the approval stands.

- 2026-09-08 (**Codex corrected Jacobian rule passes; fit retry stopped for the Director’s cost ruling**). Position-step01 found c=0.03 on a plateau at all three registered layers; original stability and six cache/reference checks pass. Source5ad9fbe carries the rule through all fitter stages. Retry01 repeated all six checks successfully, then measured layer1 batch8 broadcast at a conservative15.10hours/400 positions,5.66hours/150, beyond the25-minute gate. Current before-call memory projection excludes all larger batches. Codex stopped only its own supervisor/child after recording the cost calculation, before any full fit; primary evaluation PID84561 and lock remained live. Records and patch: `codex/lens-fitting:research/records/LENS-FIT-agentic-jacobian-2026-09-07/FIT-RETRY-01-RESULT.md`. No lens artifact, no relaxed bounds, no slot retained. Proposed next: rework batching/resource projection and benchmark before a new fit.

- 2026-09-08 10:40 local (**bake-off readiness: the evaluation path is model-agnostic, and two defects that would make a bare-id candidate look like a model that cannot stop**). From the Deputy's reading of the code, confirmed by the Chief. `spec.family` is read nowhere in `src/`; the LoRA policies carry a dense suffix set; the cache strategy resolves from the view rather than a name; the one Qwen import in `arch.py` sits in the native-capture path, not the evaluation path. So a candidate needs a registry entry and nothing else. Against that, `chat.extra_stop_tokens` is required, validated, stored and read by nothing, so every provenance snapshot including arm 1's records a control that never acted; and `_default_spec` gives any unregistered HF id Qwen's `<|im_end|>` as its turn ending, which three modules append to every rendered turn. Filed as the next issue in the Deputy's lane, ahead of any bake-off run: wire the field or delete it, and make the fallback refuse with the registry fields named. Ruled: the divergence 19 stay the competence screen for any candidate, compared on passes, pairs and lock-ins; defect classes are read fresh per model and are not part of the comparison. Codex's cost measurement for the Jacobian fit, 15.1 hours per 400 positions at layer 1, was taken while sharing the box with the evaluation and is not a valid cost; the Chief's recommendation to the Director is a benchmark alone in a gap, the regression fit as the production path, the Jacobian confined to the three registered layers at 150 positions, and the RunPod credits if the true cost still exceeds the gate. The Director's ruling is awaited on that and on the issue 86 registry entry.

- 2026-09-08 11:10 local (**the Director's pivot: Qwen3.5 retired, Gemma 3 adopted, J-space the priority; the audit, the ground truth, and what the pivot buys**). The Director's order: stop with Qwen because its architecture does not suit the instruments, run the existing evaluation suite on Gemma 3, and prioritise understanding the intermediate representations the model uses while it completes tasks. Plan recorded at `design_specifications/pending/GEMMA3-PIVOT-2026-09-08.md` (c9e4e16), from a 33-agent read-only audit run beside the Deputy's live evaluation with every load-bearing finding adversarially checked. The Director authenticated to Hugging Face and the official `google/gemma-3-4b-it` weights, config, tokenizer and chat template are on disk, so the plan rests on files rather than research. Three gains. A pre-fitted Jacobian lens for Gemma 3 exists at `neuronpedia/jacobian-lens` for every Gemma 2, 3 and 4 size, pretrained and instruction-tuned; ours is 432.5 MB, 33 maps of 2560 square, exactly `set(range(1, 34))` for a 34-layer model, in the convention `LensMaps` and `convert_jlens.py` already read. The instruction-tuned lens exists, so the pretrained-against-instruction-tuned mismatch the audit conceded is avoidable. Second, the finite-difference reference becomes unique by construction: with no recurrent state the cached and uncached responses compute the same function, which retires the self-check apparatus, the restore and broadcast dual mode, the finite-difference JVP fallback and the epsilon ceremony, and with them the section 15 step ruling and Codex's diagnostic, all of which were hybrid-specific. Third, the layer contrast improves: Gemma's `is_sliding` predicate is the same formula as Qwen's `is_linear` with period 6, so the band machinery landed this morning generalises with a rename, the derived family at 34 layers is 6, 11, 12, 17, 18, 23, 24, 28, 30, 34, and a sliding block against a global block is the same module class differing in exactly the mask window and the RoPE base, which is a near-clean single-variable contrast where the hybrid's was uninterpretable. The costs: the hand-run forward loop is wrong for Gemma in three ways, the missing square-root-of-hidden embedding scale, one mask where two are needed, and a kind predicate that cannot see the distinction, and they must land as one change together with a preflight prompt longer than the 1,024 sliding window, because both masks are identical below it and fixing the scale alone turns the residual gate green over a still-broken view. The chat template, read from the downloaded file, raises on our conversation shape: it enforces user and assistant alternation and has no tool role, so an observation at an even loop index triggers its exception. Two corrections to the Chief in the document: issue 98 is not a blocker, because MLX takes the terminator set from the config and Google ships both ids, and the fix demanded would have crashed after the weights loaded. One new hazard filed: a lens carries no model identity, and since Qwen3.5-4B and Gemma 3 4B are both 2560 wide with the Qwen lens's layers inside Gemma's count, the Qwen lens loads onto a Gemma view without complaint and produces plausible numbers. The Deputy's 1,200 full split fired before the stop message reached them and is running; it is not being stopped, and it costs nothing, because every Gemma item is off-box.

- 2026-09-08 11:25 local (**the lens-identity hazard demonstrated, and its accidental guard runs the wrong way**). Issue 99, filed this morning as a reading of `LensMaps.load`, is now shown. Both lenses are on disk: the hosted Gemma 3 4B instruction-tuned lens was downloaded, converted and verified (84db4c9), and the Qwen lens has been there since yesterday. Loading each against each model's real dimensions, Gemma's from the official config: the **Qwen lens loads onto a Gemma view with no complaint**, hash passed, thirty-one maps accepted. The reverse is refused, but only because the Gemma lens carries a map at layer 32 and Qwen has 32 layers, so the bound fails on arithmetic rather than on identity. That asymmetry is worse than no guard, because it makes the loader look protective: the direction it catches is a lens with more layers than the target, and the direction it misses is the old lens against the new, larger model, which is exactly what a change of base model invites. Pure numpy under stubs, no model loaded, beside the Deputy's running evaluation. The four lines are the acceptance test.

- 2026-09-08 11:45 local (**ruling on the architecture-view port: pay the observation forward, never memoise it, and never drive it from a mid-network residual**). The Deputy's `ARCH-VIEW-PORT-2026-09-08.md` (f971d1f) proposes fixing the port's three defects as one by observing the model rather than describing it: run the model's own call with the blocks replaced by recording stubs, and inherit the entry transform, every mask and the model's own cache selection. Approved, with one ruling and one correction. **The ruling**, which the Deputy declined to guess: one no-op forward per call is acceptable in the hot path, because the stubs skip the matmuls, so the observation's cost tracks embedding, mask construction and about thirty-four Python calls rather than model compute, and mask construction happens today regardless. Checked against the worst case rather than assumed: `cached_responses` calls `pre_norm_tail` twice per direction batch, 640 tail runs per position per layer at hidden 2560 and batch 8 inside `full_jacobian`, each running real blocks on a batch of eight, against which a stubbed one-token forward is noise. If a measurement ever says otherwise the answer is to hoist the observation to the top of `cached_responses`, where the invariant is visible, not to cache it: a key is a claim about invariance, a hoist is a proof of it. **The correction**: `gemma3_text.py:186-190` applies the square-root-of-hidden scale *unconditionally, after* the `input_embeddings` branch. So an observation driven through that argument does see the entry transform, which is good; but a mid-network residual passed the same way is multiplied by about fifty and returned as though nothing happened, and `tail` and `pre_norm_tail` hold exactly such a residual. That prohibition goes in the design, because it is the same silent-multiplication class as the defect being removed. It also sharpens the mechanism: the observation should hand the model a known vector and read what the block-0 stub received, so the entry transform is inherited as a relation rather than read as a scalar, which generalises to an additive or normalised entry and is a stronger claim than the document makes. The Deputy's fourth defect, the global mask built from `cache[pattern-1]` and the sliding mask from `cache[0]`, is accepted and is the argument for observation in one line. The negative control, breaking the dispatch deliberately and requiring the new gate to fail above the window and pass at sixty-four tokens, is kept and is not to be dropped for time.

- 2026-09-08 12:55 local (**the depth question answered, and the landing split because the architecture port strengthens what a view demands of a model**). Two things, and the second is a correction to a design the Chief approved this morning. **The Director's question, why restricting the adapter to the top eight layers turned a losing model into a winning one, is answered at `research/records/DEPTH-WHY-2026-09-08/` (9869788, 4471be9, f5bf62c, de88893).** Post-hoc depth ablation of arm A's own trained adapter, no retraining and no gradients: base 15 of 19, arm A's lower twenty-four layers alone **0**, its top eight alone 11, both together 5, and a top-eight adapter trained alone 18. So training the lower layers on this data destroys the model rather than degrading it, with no coherent completion of any task; and arm A's top-eight update improves a broken model from 0 to 5 while degrading a healthy one from 15 to 11, which is a direct measurement of co-adaptation rather than a description of it. Those layers spent their capacity compensating for the damage beneath them, which is why the same update helps in its presence and hurts in its absence, and why it lands seven tasks below the same layers trained over an intact backbone. The transferable statement: when adapting a model to a narrow behaviour, the layers you decline to train change what the layers you do train are able to learn, and that benefit is not recoverable by pruning afterwards. Earlier and weaker readings are recorded with their limits: the two arms' weight-space cosine is 0.002 against a random null of 0.001 and a same-run control of 0.92, but both write toward the same token families through the unembedding, so weight-space orthogonality is not evidence of functional difference and the ablation is what settled it. **Second: a suite over seven stacked patches gives 78 failures, of which about 48 are `ARCH-VIEW-PORT`, and they are the design's consequence rather than fixture nits.** Observing the model rather than describing it makes an `ArchitectureView` require a **callable model that accepts `input_embeddings`**, where the hand-run loop required only a block list and a way to run one block. Forty-five test doubles implement the weaker contract. The Chief approved that design without asking what it demanded of the objects it would be handed. The sharp form, for the record: the tests that break are precisely those encoding the described-model assumption, so upgrading the doubles is the honest fix and a fallback to the described path is forbidden, because it would silently restore what the port exists to delete. **Ruling: the port leaves today's stack.** The other six land together, which is the set that unblocks Codex's Gemma regression fit today via `NATIVE-RESIDUALS`, needing `PILOT-PATH` and nothing of the port; the port lands alone afterwards with its doubles upgraded, its negative control intact, and the residual-source disagreement as its acceptance number.

- 2026-09-08 13:15 local (**eight patches landed green, the architecture port among them, and the Chief's split decision withdrawn**). One suite over all eight stacked in a worktree: **1,985 passed, 14 skipped, exit 0**, fourteen skips on a two-thousand-test suite being the number that says the coverage happened rather than being reported. Landed in the verified order: 5f35206 WINDOW-RUN (issue 97), 63e4a7c PILOT-PATH, 852b9a7 LENS-IDENTITY (issue 99), b3156d2 PILOT-ARGUMENTS, 7b6c543 NATIVE-RESIDUALS, 43a50fb ARCH-VIEW-PORT, f223bdd BAND-PAIRS (issue 86), 3c8dac8 STACK-FIXES; every landed file compared byte for byte against the worktree the suite ran on. **The Chief removed the port from today's stack an hour earlier, on the ground that forty-eight failures made it multi-day. That was wrong and the Deputy's twenty minutes proved it.** The failures were the port's real consequence — observing a model rather than describing it makes an `ArchitectureView` require a callable model accepting `input_embeddings`, where the old loop needed only a block list — and the honest fix was to upgrade the doubles, which is what the Deputy did, rather than to weaken the view or to give it a fallback to the described path. The decision to split was defensible on the failure count and wrong on the diagnosis; the count was of consequences, not of causes. Also landed today from the Chief's own work: both Gemma 3 4B checkpoints converted here from Google's authenticated weights under one window, `models/gemma-3-4b-it-4bit` at 2.1 GB and `-bf16` at 7.3 GB, both verified text-only with zero vision tensors counted from the safetensors headers and both carrying `eos_token_id: [1, 106]`; record at e029857, and the conversion took under a minute for the pair against a ten-minute estimate. The registry still names the multimodal source and should name the local 4-bit for the pilot, which is the Director's to approve. Box passes to the Deputy for stage one of the mapping pilot; Codex is unblocked on both the regression and the Jacobian fit, the latter earlier than the Chief promised, because the port landed.

- 2026-09-08 (**a boolean mask cast to a float removed Gemma's sliding window, and only a comparison against the real checkpoint could see it**). The fault, in landed code and mine: `arch.masks` gave an additive mask the residual's precision, which is right, and cast **every** array, which is not. A boolean mask is a predicate — `True` means attend — and as a float it becomes an additive mask of ones and zeros, so every position is attendable and the window is gone. On `models/gemma-3-4b-it-4bit` at 1,400 tokens the residual at layer 1 diverged from the model's own forward by **95 per cent**; at 64 tokens, by 2.

  **The suite was green over it and could not have been otherwise.** Every test model in this repository returns a string sentinel or an additive mask; not one returns a boolean array, so no test exercised the branch that was wrong. And below Gemma's 1,024-token window both of its masks are the string `"causal"`, so even on the real model at every length the tests use, no array is cast and nothing shows. The transferable finding is the Chief's phrasing: **a toy that exercises one convention cannot detect a convention error**, and the only instrument that could was a comparison against the real thing at a length where the conventions differ.

  Found by the architecture port's acceptance, run on the Chief's instruction before the mapping run rather than after: the view's native-dtype loop against the model's own forward, at 64 and 1,400 tokens, with a negative control that breaks the mask dispatch. After the fix the gate is **0.0 absolute and 0.0 relative at both lengths**, and the control is **0.0 at 64 and 5.31 (5.1 per cent) at 1,400** — nothing below the window, everything above it, which is the asymmetry the design predicted and the reason the repository's 64-token residual gate could not have caught this.

  **A second defect the same run found, and it is the sharper one.** `diagnostic_native_final_residual` read `embed_tokens` directly and so never applied Gemma's entry scale. The pivot document listed it beside `embed` in one sentence; the port fixed `embed` and left it. That method **is** the residual gate's comparator, so the one place the omission would have been caught is the one place it survived — a broken loop compared against itself, 79 per cent out at 64 tokens, reporting nothing. **A gate whose comparator shares the defect it is gating measures nothing, and it reports zero.**

  Two smaller things from the same afternoon. My first negative control patched `masks`, which the hoist had taken off the gate's path, so it reproduced the uncontrolled number to the last bit — a control that cannot fail, one level down from a suite that skipped its own files; the Chief caught it in the JSON before I reported. And the first acceptance instrument compared a float32 loop against a bfloat16 forward, so its 181,895 was the promotion plus any defect, inseparably: an absolute difference against a residual whose norm nobody quoted was never going to be an acceptance. Both fixed; record at `research/records/GEMMA3-PORT-ACCEPTANCE-2026-09-08/`.
