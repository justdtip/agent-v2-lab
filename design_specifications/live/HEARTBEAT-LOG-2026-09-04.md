...
- 2026-09-05 20:22:07 onward (**the live instance of the name-matching gap**): a model-loading process ran for over twenty minutes that no session's check saw. Pid 63943, `ctxmax.py`, launched from the Research Division session's scratchpad, parent exited so orphaned to launchd, holding `mlx-community/Qwen3.5-4B-MLX-4bit` for a context-length benchmark with a physical footprint around 16.6-17.2 GB on a 26 GB box; free memory down to ~200 MB. It overlapped nothing, so the one-model rule held **by luck, not by a check**: `ctxmax.py` matches no pattern in any list, which is what a scratch script always looks like.
