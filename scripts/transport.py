...
def main() -> int:
    ...
    # The direct path: read the concept vector itself through the model's own head, with no
    # propagation at all. If this alone ranks the concept well, "it reached the output" is not
    # evidence of anything the remaining layers did.
    def direct_rank(v, true_i):
        with torch.no_grad():
            h = v.to(device=device, dtype=next(model.parameters()).dtype).unsqueeze(0).unsqueeze(0)
            lg = model.lm_head(model.model.norm(h))[0, 0].float().cpu().log_softmax(-1)
        scores = [float(lg[t]) for t in first_token]
    ...
    for k in range(args.trials):
        ...
        true_i = index[word]
        v = bank[layer][true_i]
        scale, _a, note = meter.scales_for_ladder(v, layer, (args.tier,),
        ...
        arms = {
            ...
            "permuted": permuted_in_basis(v, bank[layer], generator=g),
        }
        for arm, vec in arms.items():
            sc = scale if arm == "concept" else meter.scales_for_ladder(
                vec, layer, (args.tier,), residual_norm=norms[layer], verify=False)[args.tier][0]
            ...
            rows.append(dict(
                ...
                direct_rank=direct_rank(vec, true_i)))
        if k % 5 == 0:
    ...
    print(f"\n{'arm':<14} {'n':>4} {'transport':>10} {'carry cos':>10} {'rank':>8} "
          f"{'top-10%':>8} {'direct rank':>12}   (chance {(n-1)/2:.0f})")
