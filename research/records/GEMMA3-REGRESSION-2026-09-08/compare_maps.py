...
def metrics(candidate, reference):
    ...
    for start in range(0, d, 128):
        ...
        aa += float(np.sum(a*a)); bb += float(np.sum(b*b))
        ab += float(np.sum(a*b)); diff += float(np.sum((a-b)**2))
        identity = np.zeros_like(a)
        identity[np.arange(a.shape[0]), np.arange(start,start+a.shape[0])] = 1
        aid += float(np.sum((a-identity)**2)); bid += float(np.sum((b-identity)**2))
    return {"regression_frobenius": math.sqrt(aa), "hosted_frobenius": math.sqrt(bb),
...
def self_check():
    ...
    m = metrics(np.array([[0., 1.], [0., 0.]]), np.array([[0., 0.], [1., 0.]]))
...
def main():
    ...
    hosted = Path("/Users/daniel.tipton/Desktop/An app/models/jlens/gemma-3-4b-it_jacobian_lens.npz")
    ...
    expected = LensIdentity("gemma3-4b-bf16", "/Users/daniel.tipton/Desktop/An app/models/gemma-3-4b-it-bf16", depth)
    reg = LensMaps.load(fitted, expected_sha256=meta["npz_sha256"], hidden_size=d, num_layers=depth, identity=expected)
    ref = LensMaps.load(hosted, expected_sha256="a14ab264fc47de9c59b3183a8783a60ca6f1dee2ea635e88223d5b2d67b03193", hidden_size=d, num_layers=depth, identity=LensIdentity("gemma3-4b", "google/gemma-3-4b-it", depth))
    ...
    rows = [{"layer": layer, "normalized_depth": layer/depth,
             **metrics(reg.maps[layer],ref.maps[layer])} for layer in sorted(reg.maps)]
    result = {"purpose": "descriptive comparison of two fitting objectives; not a validation gate",
              "regression_identity": expected.as_dict(), "hosted_identity": ref.identity.as_dict(),
              "regression_sha256": reg.sha256, "hosted_sha256": ref.sha256,
              ...
              "arithmetic": "float64 reductions of stored float32 maps; hosted storage promoted from float16",
              "rows": rows, "final_layer_baseline": {"layer": depth,"normalized_depth":1.0,**metrics(np.eye(d),np.eye(d))},
...
if __name__ == "__main__":
    if "--self-check" in sys.argv:
        self_check();print("matrix metric self-check passed")
    else:
        main()
