...
# RECORD, NOT A LAUNCHER (2026-09-05). This is the script as it ran, kept for the record. It loads
# the model, so it must not be started by hand: runs go through the package entry points under R47
# and R48, in a window the Director declares, and take the model-run lock of issue 83.
import sys as _sys
if __name__ == "__main__" and "--i-am-a-record" not in _sys.argv:
    _sys.exit("refusing to run: this file is the as-run record of the 2026-09-05 training-cost "
              "probe, not a launcher; runs go through the package entry points under R47 and R48")
...
def main() -> int:
    ...
    try:
        ...
        model.freeze()
        # E: LoRA on the attention and recurrence projections only, no MLP keys, so the MLP's
        # LoRA path does not exist. F: the full key set, adapters cast to bfloat16 after
        # conversion, so the LoRA path cannot promote the projections' outputs to float32.
            ...
            for name, module in model.named_modules():
                for attr in ("lora_a", "lora_b"):
                    if hasattr(module, attr):
                        setattr(module, attr, getattr(module, attr).astype(mx.bfloat16)); casted += 1
            row["recipe"]["lora_adapter_dtype"] = "bfloat16"
            row["recipe"]["lora_tensors_cast"] = casted
        model.train()
        # C: the trainer's step with gradient checkpointing OFF. D: checkpointing on, but the
        # recurrence's backward removed by stopping gradients at the gated-delta output, which
        # is the calibration table's "floor" (preflight.py _FLOOR_MODE): what a step costs with
        # the recurrence's retained state out of the picture.
        if args.variant == "C":
            row["recipe"]["grad_checkpoint"] = False
        if args.variant == "L":
            # Half the layers checkpointed (the even-indexed ones), by class reassignment of
            # those instances to a checkpointed subclass; the call operator resolves __call__
            # on the type, so a per-instance attribute would not take.
            base = type(model.layers[0])
            fn = base.__call__

            class _Checkpointed(base):
                def __call__(self, *a, **kw):
                    def inner(params, *a2, **kw2):
                        self.update(params)
                        return fn(self, *a2, **kw2)
                    return mx.checkpoint(inner)(self.trainable_parameters(), *a, **kw)

            wrapped = 0
            for i, layer in enumerate(model.layers):
                if i % 2 == 0:
                    layer.__class__ = _Checkpointed; wrapped += 1
            row["recipe"]["grad_checkpoint"] = f"half: {wrapped} of {len(model.layers)} layers (even indices)"
        elif row["recipe"]["grad_checkpoint"]:
        ...
        loss = chunked_loss if args.variant == "B" else default_loss
        value_and_grad = nn.value_and_grad(model, loss)

        def one_step():
            if args.variant in ("J", "K"):
                # Forward only, no gradient: J in training mode (the recurrence's non-kernel
                # path, checkpoint wrappers in place), K in inference mode (the fused kernel).
                if args.variant == "K":
                    model.eval()
                lvalue, ntoks = loss(model, batch, lengths)
                mx.eval(lvalue)
                return float(lvalue), int(ntoks)
            (lvalue, ntoks), grad = value_and_grad(model, batch, lengths)
            if args.variant == "G":
                # Staged evaluation: force only the LAST layer's gradients first. If MLX schedules
                # every checkpointed layer's recompute as soon as it is ready, the peak reaches
                # the full step value here; if the recompute is sequential with the gradient
                # chain, this stage holds one layer's worth and the rest arrives later.
                import mlx.utils as mu
                flat = dict(mu.tree_flatten(grad))
                last = max(int(k.split("layers.")[1].split(".")[0]) for k in flat if "layers." in k)
                tail = [v for k, v in flat.items() if f"layers.{last}." in k]
                mx.reset_peak_memory()
                mx.eval(tail)
                row.setdefault("staged", []).append({
                    "stage": f"grads of layer {last} only", "peak_gib": mx.get_peak_memory() / GIB,
                    "active_gib": mx.get_active_memory() / GIB})
                mid = last // 2
                mx.eval([v for k, v in flat.items() if f"layers.{mid}." in k])
                row["staged"].append({
                    "stage": f"grads through layer {mid}", "peak_gib": mx.get_peak_memory() / GIB,
                    "active_gib": mx.get_active_memory() / GIB})
        ...
        def recurrence_backward_removed():
            module = importlib.import_module("mlx_lm.models.gated_delta")
            original = module.gated_delta_ops

            def stopped(q, k, v, g, beta, state=None, mask=None):
                out, new_state = original(q, k, v, g, beta, state, mask)
                return mx.stop_gradient(out), new_state

            module.gated_delta_ops = stopped
            try:
                yield
            finally:
        ...
        stack.enter_context(install_chunkwise_gated_delta(chunk))
        if args.variant == "D":
            stack.enter_context(recurrence_backward_removed())
            row["recipe"]["recurrence_backward"] = "removed (stop_gradient at gated_delta_ops output)"
        with stack:
            mx.reset_peak_memory()
