"""Build the review from the verified source-derived analysis; no model imports."""
from pathlib import Path
from analyze import analyze

ROOT = Path(__file__).resolve().parent
x = analyze()
def best(precision, layer):
    return next(g for g in x["best_medians"] if (g["precision"],g["layer"]) == (precision,layer))
rows = []
for layer in (1,17,33):
    f,b = best("float32",layer),best("native",layer)
    rows.append(f"| {layer} | {f['k']} | {f['median_relative_error']:.7g} | {f['max_relative_error']:.7g} | {f['projected_set_L2_error']:.7g} | {b['median_relative_error']:.7g} (k={b['k']}) |")
table = "\n".join(rows)
text = f"""# Width-one ladder audit: convergence supported, protocol acceptance incomplete
...
print("README.md regenerated from verified analysis")
