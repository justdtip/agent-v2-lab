...
ap.add_argument("--rows-from", type=int, default=0, help="first corpus row (global index) of this capture"); ap.add_argument("--rows-to", type=int, default=None, help="one past the last corpus row; default all")
...
ap.add_argument("--sample-ids-file", default=None, help="JSON list of global row indices to treat as the sample (a tail re-capture keeps the original sample rows)")
args = ap.parse_args()
...
rows_all = rows; LO = int(args.rows_from); HI = int(args.rows_to) if args.rows_to is not None else len(rows_all)
...
fam = {}
for k, r in enumerate(rows): fam.setdefault(r["metadata"]["family"], []).append(LO + k)
sample = set()
if args.sample_ids_file:
    sample = {int(i) for i in json.loads(Path(args.sample_ids_file).read_text())}; sample = {i for i in sample if LO <= i < HI}
elif not args.skip_sample:
    for f, idxs in sorted(fam.items()):
        idxs = sorted(idxs, key=lambda i: (rows_all[i]["metadata"]["task_id"], rows_all[i]["metadata"]["step"]))
        short = [i for i in idxs if len(tok(rows_all[i]["prompt"], add_special_tokens=False).input_ids) <= args.sample_max_tokens]
        step = max(1, len(short) // args.sample_per_family); sample.update(short[::step][: args.sample_per_family])
emit("sample", n=len(sample), per_family=args.sample_per_family, max_tokens=args.sample_max_tokens, from_file=bool(args.sample_ids_file))
sample_out = (OUT / "sample.jsonl").open("w"); index_out = (OUT / "index.jsonl").open("w")  # written as rows complete (a death keeps every finished row)

with torch.no_grad():
    for k, r in enumerate(rows):
        i = LO + k  # global corpus index, the key every analysis uses; k is the row in this capture's memmaps
        ...
        if k % 100 == 0 or k == N - 1:
            emit("row", n_done=k + 1, of=N, rows_from=LO, s_per_row=round((time.monotonic() - T0) / (k + 1), 2), peak_gib=round(torch.cuda.max_memory_allocated() / 2**30, 2) if args.device.startswith("cuda") else None)
