#!/bin/bash
# v2: the 12B workspace passes, gated on the Chief's GO file so they cannot auto-start into the D-CRO's
# timed slot (repeat gate, ladder) after chunk c3 and the merge. Create /workspace/chief/GO-12B to release.
set -u
...
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True  # memory management only: fewer fragmentation-driven allocation retries on a shared card
cd /workspace/chief
...
status "12b passes: merged lens present; waiting for /workspace/chief/GO-12B (the D-CRO's timed slot goes first)"
until [ -f /workspace/chief/GO-12B ]; do sleep 60; done
