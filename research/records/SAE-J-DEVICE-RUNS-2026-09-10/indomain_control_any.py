...
usage: indomain_control_any.py CONFIG LAYERS(comma) OUT_JSON SNAPSHOT CORPUS_MANIFEST LENS_DIR DICT_SNAPSHOT_ROOT RECEIPT_ROOT
(the generalised form of the 4B record script indomain_control.py: the same computation, paths as arguments)
"""
...
S4 = Path(sys.argv[4]); CORPUS = Path(sys.argv[5]); LENS_DIR = Path(sys.argv[6]); DICT_ROOT = Path(sys.argv[7]); REC_ROOT = Path(sys.argv[8])
MAX_SEQ = 128; POSITIONS = [16, 44, 71, 99, 126]
budget = B.ErrorBudget.from_dict(conf["error_budget"]); t0 = time.time()
lens, metadata = load_admitted_device_lens(LENS_DIR / "admitted-maps.npz", checkpoint=S4, model_base=conf["model_base"], expected_sha256=conf["lens_sha256"])
