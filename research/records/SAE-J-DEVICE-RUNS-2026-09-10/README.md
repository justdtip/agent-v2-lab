...
| random, same norm: valid calls with the tool changed / malformed outputs (of 128) | 2 / 0 | 15 / 0 | 0 / 0 | 15 / 1 |
| angle-matched: valid calls with the tool changed / malformed outputs (of 128) | 0 / 0 | 13 / 0 | 0 / 0 | 12 / 1 |

The first-token result of §4.1 carries through to the call: every first-token flip is a different tool call, five of the nine at layer 18 also change the path, and no arm produces malformed JSON. At layer 18 on close decisions the reconstruction changes the call in 28% of cells against 12% for random and 10% for angle-matched errors of the same size (eight of the nine go to `list_files` from `read_file` or `search_files`; one goes from the model's own `replace_text` to the expert's `read_file`). At layer 24 the reconstruction changes no call in either sample while random errors change 15 of 128 low-margin calls validly and produce one malformed output, which is counted apart: a malformed call matters behaviourally but is not a changed complete call. Where the model has a margin, nothing changes anywhere.
...
**Experiment 2 inverts the amplification reading, at both layers.** A random direction of the same norm as the dictionary's error is amplified by the readout about twice as much as the dictionary's error is: its share is above one (the readout of the error exceeds the readout of the whole activation), and the actual error ranks lowest of all sixteen matched controls in every one of the 600 cells. The "5–11× amplification" of §3 is what this gain-only vocabulary readout does to the sampled norm-matched and angle-matched random errors — sixteen per cell, 9,600 per layer — and the dictionary's error is amplified less than every one of them; what the readout does to errors from other distributions is not measured here. The review's second branch holds: the headline amplification described the readout's sensitivity, not an unusual property of the dictionary's error.

**The same positions, two instruments.** Joining §4.1 and §4.3 on the 60 unique action positions the substitution test used (the two samples overlap in four cells; these include the deliberately selected low-margin cases and are not a population sample): the lens-score argmax survives the reconstruction in 40 of 60 at layer 18 and 28 of 60 at layer 24; the model's own argmax survives in 51 of 60 and 60 of 60. That is the case for keeping lens-score fidelity and behavioural fidelity as separate outcomes, and for not letting either stand in for the other.
...
| 262k small | 0.095 / 0.67 / 7.2 / 22 / 91 | 0.150 / 0.65 / 4.4 / 19 / 95 | 0.076 / 0.79 / 10.3 / 24 / 5 | 0.179 / 0.71 / 3.9 / 5 / 2 |
